...
if (! function_exists('is_digits')) {/*is the param is digit or not*/	function is_digits($element) {		return !preg_match ("/[^0-9]/", $element);	}}
// -----------------------------------------------------------------------------------------------------------------
/**
 ...
 * Currency format
 * 
 * @param $floatcurr currency value
 * @return return give currency based format.
 */
  
if (! function_exists('formatcurrency')) {
 function formatcurrency($floatcurr, $curr = "NOK")
 {
...
/**
 * Currency format for INR
 * 
 * @param $input currency value
*/
...
function addDays($startDate,$numDays,$status)
{
			$endDate = $startDate;
			$counter = 1;
			while ($counter <= $numDays) {
			   
			   $endDate = date("Y-m-d", strtotime(date("Y-m-d", strtotime($endDate)) . " +1 day"));
			   $dayOfTheWeek = date("l",strtotime($endDate));
			   
			  // $dayOfTheWeek == "Saturday" || 
			if ($dayOfTheWeek == "Sunday" && $status) {
				   continue;
			   }else {
				  $counter++;
			   }
