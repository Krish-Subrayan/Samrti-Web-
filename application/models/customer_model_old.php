...
class Customer_model extends Super_Model
{
    ...
    function searchCustomer($keyword='',$orderby = 'id', $sort = 'ASC',$type ='name')
    {
		...
		if($type == 'name'){
			...
			if($current_partner_branch != 14){
				...
				$additional_sql .= " CONCAT(IFNULL(name,''),' (',IFNULL(p.number,''),')') AS name ";
				$tbl = "a_company";
        ...
        $query = $this->db->query("SELECT a_company.id, $additional_sql ,p.number,e.email
                                          FROM a_company
										  LEFT JOIN a_company_customer as cc ON cc.company = a_company.id AND cc.status='1'
										  LEFT JOIN a_customer as c ON c.id = cc.customer
	...
	function companycustomermobileinfo($mobile)
	{
		...
		$query=$this->db->query("SELECT b.firstname,b.lastname,b.sex,c.email,a.number as mobile,a.customer as id,d.street_line_1,d.street_line_2,d.floor,d.calling_bell,d.zip,p.name as partner_name,pb.name as partner_branch_name,a_company_customer.type,a_company_customer.company
		FROM a_phone a 
		LEFT JOIN a_customer as b ON a.customer=b.id
	...
	{
		$sql="SELECT * FROM a_company WHERE status='1'";
		$query=$this->db->query($sql);
	...
	function validateCompanycustomer($cus_id)
	{
		$sql="SELECT * FROM a_company_customer WHERE customer='".$cus_id."'";
		$query=$this->db->query($sql);
		$status=$query->num_rows();
		return $status;
