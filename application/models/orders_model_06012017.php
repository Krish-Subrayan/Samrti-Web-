...
class Orders_model extends Super_Model{
    ...
    function __construct()
    {
		...
		/* Get customer order history*/
		function getCustomerOrderhistory($customer,$type ='all')
		{
			$conditional_sql='';
			$add_sql='';
...
ON a_order.id=a_order_log.order WHERE a_order.id='".$id."' order by a_order_log.status desc limit 0, 1");
    ...
    function getOrderLine($id='')
    {
        ...
        $this->benchmark->mark('code_start');
		//_____SQL QUERY_______
        $query = $this->db->query("SELECT *,a_orderline.price as price,a_orderline.id as id,DATE_FORMAT(a_orderline.p_b_delivery_time,'%d.%m.%Y') as utlevering,a_product.description,CONCAT('".PATH_IMAGE_FOLDER."',a_images.path) as path, a_product.type as ptype,a_product.name FROM a_orderline 
		LEFT JOIN a_product ON a_product.id=a_orderline.product 
	...
	/*update utlevering*/
	function updateOrderline($orderline,$orderid,$changed_quantity,$price,$total)
	{
...
//send email now
 ...
 $query = $this->db->query("INSERT INTO email_log (
	...
	function orderlinePayment($orderid,$orderline,$payment_type,$payment_status)
	{	
		$addsql="";
	...
	function orderlineHeatseal($id='')
	{
        ...
        $this->benchmark->mark('code_start');
		//_____SQL QUERY_______ 
        $query = $this->db->query("SELECT a_product.name,a_orderline.id as id,a_orderline.amount,DATE_FORMAT(a_orderline.p_b_delivery_time,'%d.%m.%Y') as utlevering,		a_product.description,a_product.type as ptype,a_heat_seal.heat_seal_barcode	 as barcode,a_heat_seal.id as heatid,a_heat_seal.additional_product
		FROM a_orderline 
		LEFT JOIN a_product ON a_product.id=a_orderline.product 
