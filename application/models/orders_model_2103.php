...
ON a_order.id=a_order_log.order WHERE a_order.id='".$id."' order by a_order_log.status desc limit 0, 1");*/
	...
	$query = $this->db->query(" SELECT a_order_log.status as order_status,a_order.*,total_amount,changed_amount,DATE_FORMAT(a_order.order_time,'%d.%m.%Y %H:%i:%s') as order_time
		FROM a_order 
		LEFT JOIN a_order_log
					...
					if($this->session->userdata['partner_branch'] == 14 || $this->session->userdata['partner_branch'] == 1000 || $this->session->userdata['partner_branch'] == 28)
					{
						...
						if($proqty == 1)
						{
							if($cartitem['options']['heatseal'] != '')
							{
								...
								$qry="INSERT INTO a_heat_seal_log SET `heat_seal`='".$heatsealid."',`orderline`='".$orderline."',`status`='added',`regtime`='".date('Y-m-d H:i:s')."',employee_p_branch='".$this->session->userdata('employee_p_branch')."',customer='".$customer."',company='".$this->session->userdata['customer']['company']."'";	
								$this->db->query($qry);
								//$this->db->insert_id();
							}
