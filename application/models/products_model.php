...
class Products_model extends Super_Model
{
    ...
    function getCategories($id='')
    {
		...
		$current_partner=$this->session->userdata['partner'];
		 ...
		 if($this->session->userdata['logged_in'] !='shop'){
       		...
       		$query = $this->db->query("SELECT c.id,c.name,c.slug,c.description,c.full_screen, 
									   	  CONCAT('".PATH_IMAGE_FOLDER."',i.path) as path,(SELECT COUNT(category) from a_product as p WHERE p.category = c.id AND p.status=1 AND p.s_laundry=1 GROUP by category ) as count 
                                          FROM   a_category as c
        ...
        $query = $this->db->query("SELECT c.id,c.name,c.slug,c.description,c.bg_color,(SELECT COUNT(category_partner) from a_product_p_branch_category WHERE category_partner = c.id AND c.partner ='".$current_partner."' GROUP by  category_partner) as count 
    ...
    function getParentCategories($id=''){
	  ...
	  END  AS name 
	  from  a_product_p_branch_category as pbc ,a_product as p
	  where pbc.id = $id AND p.id = pbc.product_p_branch");
    ...
    function getProduct($id='',$imgtype='main',$cat='',$type='shop',$subscription='1',$parent=0)
    {
		...
		if($cat!=''){
			$conditional_sql .= " AND pbc.category_partner=$cat";
		}
			
		if($id==''){
			$conditional_sql .= " AND pbc.parent=$parent";
		}
		
		$conditional_sql .= " AND pbc.status='1'";
	...
	END AS price,";
			...
			if(intval($current_partner_branch) == 14 || intval($current_partner_branch) == 28 || $this->session->userdata['company_status']) //hvittsnip branch  ( only this branch accept orders from comapny)
			{
		...
		$sql="SELECT p.id,p.name,pb.heatseal,p.in_meter,pb.note,pb.cleaning_duration as duration,p.description,o.price as offer_price,o.percent$img,c.name as category,c.slug as cslug,c.id as cid,p.alert,p.type as gtype,pbc.id as pbcid,pbc.bg_color,
	$pricesql
	  CASE 
	  WHEN pbc.display_name IS NULL THEN p.name 
	  WHEN pbc.display_name ='' THEN p.name 
	  ELSE pbc.display_name
	  END  AS name, (SELECT COUNT(id) FROM  a_product_p_branch_category as ppbc WHERE ppbc.parent = pbc.id AND ppbc.status='1' ) as child
	  
                                          FROM a_product_p_branch_category as pbc,a_product_p_branch as pb, a_category_partner as c,a_product as p
										  ...
										  AND pbc.category_partner=c.id
										  AND p.id = pbc.product_p_branch
										  $conditional_sql
                                          ORDER BY pbc.sort_order ASC";
        ...
        $query = $this->db->query("SELECT p.id,p.name,ROUND(p.price, 0) as price,p.description,ROUND(o.price, 0) as offer_price,o.percent$img,c.name as category,c.slug as cslug,c.id as cid,p.alert as note,p.type as gtype
                                          FROM a_category as c,a_product as p
										  LEFT JOIN a_offer as o ON o.product = p.id AND start_time <= NOW() AND end_time >= NOW() AND o.status='1' 
										  $additional_sql
										  WHERE 1=1 $conditional_sql
										  AND c.id=p.category
    ...
    function getPartnerProductList($id='',$imgtype='main',$cat='',$type='shop',$subscription='1')
    {
	...
	END AS price,";
        ...
        $query = $this->db->query("SELECT DISTINCT p.id,p.name,pb.heatseal,p.in_meter,pb.note,pb.cleaning_duration as duration,p.description,o.price as offer_price,o.percent$img,c.name as category,c.slug as cslug,c.id as cid,p.alert,p.type as gtype,pbc.id as pbcid,pbc.bg_color,
	$pricesql
	  CASE 
	  WHEN pbc.display_name IS NULL THEN p.name 
	  ELSE pbc.display_name
   ...
   ELSE pb.price 	  
END  AS price
                                          FROM a_product_p_branch as pb,a_category as c,a_product as p
										  LEFT JOIN a_offer as o ON o.product = p.id AND start_time <= NOW() AND end_time >= NOW() AND o.status='1' 
										  $additional_sql
										  WHERE 1=1 $conditional_sql
										  AND pb.product= p.id AND pb.status=1 AND pb.partner_branch ='".$current_partner_branch."' 										  
        ...
        $query = $this->db->query("SELECT p.id,p.name,pb.heatseal,pb.note,pb.cleaning_duration as duration,p.description,o.price as offer_price,o.percent$img,c.name as category,c.slug as cslug,c.id as cid,p.alert,p.type as gtype,pbc.id as pbcid,pbc.bg_color,
	$pricesql
	  CASE 
	  WHEN pbc.display_name IS NULL THEN p.name 
	  ELSE pbc.display_name
	  END  AS name, (SELECT COUNT(id) FROM  a_product_p_branch_category as ppbc WHERE ppbc.parent = pbc.id ) as child
    ...
    function getProduct_Discount($id='0')
    {
		...
		if($current_partner_branch == 1000){
			
			$conditional_sql.= " AND d.s_laundry=1"; 
		}
		else{
			$conditional_sql.= " AND d.partner_branch=$current_partner_branch"; 
		}
...
END  AS price
	...
	/* Get additional Product Count*/
	function additionalProductCount($product)
	{
        ...
        $query = $this->db->query("SELECT * FROM a_product_additional_product WHERE product='".$product."'");
    ...
    function getProductinfo($id='',$imgtype='main',$cat='',$type='shop',$subscription='1',$parent=0)
    {
