...
class Payments_model extends Super_Model
{
    ...
    function addCustomerPayment($paymentarray)
    {
		...
		if(isset($paymentarray['order']))
		{
			...
			$add_sql.=" AND `order`='".$paymentarray['order']."'";
		}
	...
	function updateCustomerBalance($cusotmer,$amount,$in_status='',$type='credit')
	{
		...
		if(count($results) > 0 ){
			...
			$qry="INSERT INTO a_customer_account_partner SET 
			 $filed='".$amount."',
			 customer='".$cusotmer."',partner ='".$current_partner."'";
	...
	function getSaldostatus($customer)
	{
		//profiling::
        $this->debug_methods_trail[] = __function__;
