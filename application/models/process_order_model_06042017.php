...
JOIN a_collection_time AS ct2 ON a_order.delivery_time = ct2.id
	...
	function heatsealbarcodestatus($barcode,$orderline)
	{
		...
		if($query->num_rows() > 0)
		{
			...
			$query=$this->db->query("SELECT * FROM a_heat_seal_log WHERE heat_seal='".$heat_sealid."' ORDER BY id DESC LIMIT 0,1");
			if($query->num_rows() > 0)
			{
				$results = $query->row_array(); //multi row array
			...
			{
					$query=$this->db->query("SELECT * FROM a_heat_seal_log WHERE heat_seal='".$heat_sealid."' AND orderline='".$orderline."' AND status='2'");//started
					if($query->num_rows() > 0)
					{
						return true;
					}
...
a_order.order_time,a_order.type,a_order.partner_branch,a_order.changed_amount,a_order.total_amount
		...
		LEFT JOIN a_order ON a_order.id=a_orderline.order
		WHERE a_heat_seal.heat_seal_barcode='".$heatseal."' AND a_order.id=a_orderline.order AND a_heat_seal_log.status='2' ORDER BY a_heat_seal_log.id DESC";
	...
	{
			$sql="SELECT a_product_additional_product.additional_product as id,a_additional_product.name FROM 
	...
	/*craeteInfile*/ 
	function craeteInfiledownload()
	{

		$addsql="";

		$orderids=array('05032450');
		if(count($orderids) > 0)
		{
		...
		//$addsql .=" AND DATE(a_order.order_time) = '2017-03-28'"; 
		
	
			$query=$this->db->query("SELECT a_order.id AS orderid,a_order.order_time,a_customer.firstname,a_customer.lastname,CONCAT(IFNULL(a_customer.firstname,''),', ',IFNULL(a_customer.lastname,'')) AS customer_name,a_orderline.regtime,a_orderline.p_b_delivery_time,a_heat_seal.heat_seal_barcode,
	a_phone.number as phone,a_product.name,
	CASE
	...
	LEFT JOIN a_product ON a_product.id=a_orderline.product
	WHERE a_order.type='shop' AND a_heat_seal_log.id in (SELECT MAX(id) FROM `a_heat_seal_log` WHERE 1=1 AND a_orderline.id=a_heat_seal_log.orderline GROUP BY `heat_seal`) $addsql ORDER BY a_order.id");
