...
END  AS oprice
	...
	function getCancelOrderhistory($orderid=0,$type='order')
	{
	//	$oids=implode(',',$orderids);
		if(intval($orderid) > 0)
		{
		...
		//$conditional_sql=" AND a.id IN(".$oids.")";
		$orderby=" GROUP BY a.id ORDER BY a_order_canceled.id DESC";
		
			 
			if($type =='orderline'){ 
				$additional_sql = ' a_order_canceled.orderline=a_orderline.id AND a_orderline.order=a.id AND';
			}
			
			if($type =='order'){ 
				$additional_sql = ' a_order_canceled.order=a.id AND a_orderline.order=a.id AND';
			}
			...
			p.name as partner,
			pb.name as partner_branch,
			a_order_canceled.id as cid,
			CONCAT(IFNULL(a_customer.lastname,''),', ',IFNULL(a_customer.firstname,'')) AS customername,
CONCAT(IFNULL(e.fname,''),' ',IFNULL(e.mname,''),' ',IFNULL(e.lname,'')) AS ename,CONCAT(IFNULL(SUBSTR(e.fname, 1, 1),''),IFNULL(SUBSTR(e.mname, 1, 1),''),IFNULL(SUBSTR(e.lname, 1, 1),'')) AS einitial,
a_order_canceled.canceled_reason,
a_order_canceled.comment
			FROM a_order_canceled,a_orderline ,a_order as a
			LEFT JOIN a_employee as e ON e.id = a.employee
			LEFT JOIN a_employee_p_branch as epb ON e.id = epb.employee 
			LEFT JOIN a_partner_branch as pb ON a.partner_branch=pb.id
			LEFT JOIN a_partner as p ON pb.partner=p.id
			...
			WHERE 1=1 AND 
			$additional_sql
			 (a_order_canceled.id in (SELECT id FROM a_order_canceled WHERE id IN(SELECT max(id) as id FROM a_order_canceled GROUP BY `order`) AND `status`= 'pending' ORDER by id DESC) OR a_order_canceled.id in (SELECT id FROM a_order_canceled WHERE id IN(SELECT max(id) as id FROM a_order_canceled GROUP BY `orderline`) AND `status`= 'pending' ORDER by id DESC))
			 $conditional_sql $orderby";
			...
			if($result['status'] == 'pending')
			{
				...
				if(count($orderdetails) > 0)
				{
					foreach($orderdetails as $orderlineinfo)
					{
						...
						$qry="UPDATE a_orderline SET payment_status='canceled',changed_quantity='0',changed_amount='0' WHERE id='".$orderline."' AND `order`='".$orderid."'";
						$query=$this->db->query($qry);
				   ...
				   //update payment status
					if($orderinfo['payment_status'] == 'paid' )
					{
						...
						'amount'=>$totalamount,
						'regtime'=>$regtime,
						'complaint_reason'=>'Order canceled');
						...
						$result = $this->payments_model->updateCustomerBalance($customer,$totalamount,$in_status,'credit');
						
					}
					
				//update order amount	
				$qry="UPDATE a_order SET payment_status='canceled',changed_amount='0',tax='0' WHERE id='".$orderid."'";
				$query=$this->db->query($qry);
		...
		else
		{
				...
				foreach($orderdetails as $orderlineinfo)
				{
					...
					if($query->num_rows() > 0)
					{
						...
						if($result['status'] == 'pending')
						{
							...
							$orderline_arr=$this->orders_model->getOrderlineinfo($orderid,$orderline);
							
							$amount = ($orderline_arr['changed_amount']!='') ? $orderline_arr['changed_amount'] : $orderline_arr['amount'];
							
							
							if($orderline_arr['payment_status'] == 'paid')
							{
								$type='in';
								$in_type='complaint';
								$in_status='paid';
								...
								'regtime'=>$regtime,
								'order'=>$orderid,
								'complaint_reason'=>'Orderline canceled');
								$saldoid=$this->payments_model->addCustomerPayment($paymentarray);
							...
							$this->orders_model->updateOrderlinelog($orderline,11);
			...
			if(count($orderdetails) > 0)
			{
				...
				if($status == count($orderdetails))
				{
					...
					if(trim($orderinfo['changed_amount']) != '')
					{
						$total_amount=$orderinfo['changed_amount'];
					}
					
					$total_amount = $total_amount - $cancel_amount;
					$mva=$total_amount/1.25;	
					$mva=round($mva, 2);
					$mva=$total_amount-$mva;
					
					//update order amount	
					$qry="UPDATE a_order SET  changed_amount='".$total_amount."',tax='".$mva."' WHERE id='".$orderid."'";
					$query=$this->db->query($qry);
				}
	...
	function getCancelOrderLine($order_id,$odlines,$status='pending')
	{
		...
		$query=$this->db->query("SELECT id FROM a_order_canceled WHERE `order`=".$order_id." $addsql AND id IN (SELECT MAX(`id`) FROM a_order_canceled WHERE `order`=".$order_id." $addsql ORDER BY id DESC)");
		...
		else
		{
			...
			$query=$this->db->query("SELECT orderline FROM a_order_canceled WHERE status ='".$status."' AND orderline IN  (".implode(',',$odlines).")");
	...
	/*orders deliver on the same day order placed*/
	function sameDayOrderDeliver()
	{
		$query=$this->db->query("SELECT ol.order FROM a_order_log AS ol WHERE DATE( regtime ) in ( SELECT DATE( regtime ) FROM a_order_log WHERE STATUS = '3' AND a_order_log.order = ol.order ) AND STATUS = '9'");
		if($query->num_rows() > 0)
		{
			$result=$query->result_array();
			return $result;
