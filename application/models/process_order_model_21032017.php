...
JOIN a_collection_time AS ct2 ON a_order.delivery_time = ct2.id
	...
	{
		$query=$this->db->query("SELECT * FROM a_heat_seal_log WHERE orderline='".$orderline."' AND status='started'");		
	...
	function heatsealbarcodestatus($barcode,$orderline)
	{
		...
		if($query->num_rows() > 0)
		{
			...
			{
				$results = $query->row_array(); //multi row array
				if($results['status'] == 'finished' || $results['status'] == 'canceled')
				{
					return false;
				}
				else
			...
			{
					$query=$this->db->query("SELECT * FROM a_heat_seal_log WHERE heat_seal='".$heat_sealid."' AND orderline='".$orderline."' AND status='started'");
					if($query->num_rows() > 0)
					{
						return true;
					}
...
a_order.order_time,a_order.type,a_order.partner_branch,a_order.changed_amount,a_order.total_amount
	...
	function downloadInfile()
	{
		...
		/*$query=$this->db->query("SELECT v.*, (SELECT COUNT(id)FROM vinderen WHERE `TikDetTikNum`=v.TikDetTikNum) as ocount FROM vinderen as v WHERE 1=1 AND (Left(TikDetTikNum,2) = '28' OR Left(TikDetTikNum,2) = '14') AND status='inserted' ");*/
