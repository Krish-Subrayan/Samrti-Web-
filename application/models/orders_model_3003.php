...
ON a_order.id=a_order_log.order WHERE a_order.id='".$id."' order by a_order_log.status desc limit 0, 1");*/
	...
	$query = $this->db->query(" SELECT a_order_log.status as order_status,a_order.*,total_amount,changed_amount,DATE_FORMAT(a_order.order_time,'%d.%m.%Y %H:%i:%s') as order_time,DATE_FORMAT(a_order_log.regtime,'%d.%m.%Y %H:%i:%s') as deliverytime
		FROM a_order 
		LEFT JOIN a_order_log
