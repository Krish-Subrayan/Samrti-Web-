...
class Customer_model extends Super_Model
{
	...
	function addAutofilInvoice($cus_id,$faktura_value)
	{
		...
		{
			$qry="UPDATE a_customer SET autofil_invoice	='".trim($faktura_value)."' WHERE id='".$cus_id."'";
		}
		else
		{
			$qry="UPDATE a_customer SET autofil_invoice=NULL WHERE id='".$cus_id."'";
		}
	...
	{
			$qry="SELECT a_customer.id,a_customer.firstname,a_customer.lastname,a_customer.sex,a_email.email,a_address.street_line_1,a_address.street_line_2,a_address.floor,a_address.calling_bell,a_address.zip,a_city.city,a_phone.number as phone,a_phone.number as mobile,a_customer.dob,a_email.verified as emailverified,a_company_customer.type,a_company_customer.company,a_customer.autofil_invoice FROM a_customer 
			...
			WHERE  a_customer.id='".intval($cus_id)."'";
			 ...
			 $results = $query->row_array();
			  if ($query->num_rows() > 0) 
			  {
					if($results['autofil_invoice'] != '')
					{
						$results['autofil_invoice']=intval($results['autofil_invoice']);
					}
					return $results;
			  } else {
					return false;
	...
	function customermobileinfo($mobile)
	{
		...
		if(intval($total) > 0)
		{
			...
			{
						$response['autofil_invoice']=intval($response['autofil_invoice']);
			}
			return $response;
	...
	{
		$query=$this->db->query("SELECT autofil_invoice FROM a_customer WHERE id='".trim($customer)."'"); 
		$total=$query->num_rows();
		$amount='';
		if(intval($total) > 0)
		{
