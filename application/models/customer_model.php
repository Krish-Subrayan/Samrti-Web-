...
class Customer_model extends Super_Model
{
	...
	function getCustomerAccountLog($customer='',$type='',$in_type='',$in_status='',$date='',$account='1'){
			...
			$conditional_sql .= " AND a.in_type !='complaint'";
			}
			else{
			...
			if($date != '')
			{
				$conditional_sql .= " AND DATE(`a`.`regtime`)='".date('Y-m-d',(strtotime($date)))."'";
			}
			...
			$qry="SELECT a.*,DATE_FORMAT(a.regtime,'%d.%m.%Y') as rdate,TIME_FORMAT(a.regtime, '%H:%i') as rtime,TIME_FORMAT(a.regtime, '%H:%i:%s') as rtimews,DATE_FORMAT(a.regtime,'%d/%m/%y') as rdatewy,
				  ...
				  FROM a_customer_account_partner as cap,a_customer_account_log as a
				  ...
				  $add_sql
				  WHERE 1=1
				  AND cap.partner = '".$current_partner."' AND a.customer = cap.customer
	...
	function getCustomerpayment($customer='',$type='',$in_type='',$in_status='',$date='')
	{
		...
		$add_sql='';
			...
			$conditional_sql .=" AND a_order.partner = '".$current_partner."' AND a_order.partner_branch = '".$current_partner_branch."'";
			
			//$orderby = 'ORDER BY a_orderline.modtime ASC';
			
			$orderby = 'ORDER BY a_order.id ASC';
		...
		CASE 
				  ...
				  a_orderline.modtime,a_orderline.regtime as regtime,
				...
				DATE_FORMAT(a_orderline.modtime,'%d.%m.%Y') as rdate,TIME_FORMAT(a_orderline.modtime, '%H:%i') as rtime,TIME_FORMAT(a_orderline.modtime, '%H:%i:%s') as rtimews,DATE_FORMAT(a_orderline.modtime,'%d/%m/%y') as rdatewy,
				  ...
				  $add_sql
				  WHERE 1=1 AND a_order.partner='".$current_partner."' AND a_order.partner_branch='".$current_partner_branch."' AND a_orderline.payment_type != '' AND (a_orderline.payment_status = 'paid' OR a_orderline.payment_status = 'waiting') AND
			...
			if(count($result) > 0)
			{
				foreach($result as $key=>$resitems)
				{
					//.$resitems['product']
					$newid=md5($resitems['order'].$resitems['in_type']);
					if(!isset($newarray[$newid]))
					{
						$newarray[$newid]=$key;
					}
					else
					{	
						$result[$newarray[$newid]]['amount']+=$resitems['amount'];
						unset($result[$key]);
					}
    ...
    function searchCustomer($keyword='',$orderby = 'id', $sort = 'ASC',$type ='name')
    {
		...
		if($type == 'name'){
			...
			if($current_partner_branch != 14 && $current_partner_branch != 28){
	...
	function addAutofilInvoice($cus_id,$faktura_value)
	{
		$current_partner = $this->session->userdata['partner'];
		$current_partner_branch=$this->session->userdata['partner_branch'];
		$conditional_sql.=" AND partner = '".$current_partner."' AND partner_branch = '".$current_partner_branch."'";
			
		if(intval($faktura_value) > 0)
		{
			$qry="UPDATE a_customer_account_partner SET autofil_invoice	='".trim($faktura_value)."' WHERE customer='".$cus_id."' $conditional_sql";
			
			$this->db->query($qry);
		}
		else
		{
			$qry="UPDATE a_customer_account_partner SET autofil_invoice	=NULL WHERE customer='".$cus_id."' $conditional_sql";
			$this->db->query($qry);
		}
	...
	function addCustomer($fields,$cid)
    {
		if(intval($cid) > 0)
		{
			...
			{
				if(trim($fields['employee']) != '')
				{
					$sql="SELECT employee FROM a_customer WHERE id='".trim($cid)."'";
					$query=$this->db->query($sql);
					$results = $query->row_array();
				
					if(intval($results['employee']) == 0)
					{
						$qry="UPDATE a_customer SET 
	...
	function getCustomerinfo($cus_id)
	{
			...
			$qry="SELECT a_customer.id,a_customer.firstname,a_customer.lastname,a_customer.sex,a_email.email,a_address.street_line_1,a_address.street_line_2,a_address.floor,a_address.calling_bell,a_address.zip,a_city.city,a_phone.number as phone,a_phone.number as mobile,a_customer.dob,a_email.verified as emailverified,a_company_customer.type,a_company_customer.company,
			a_customer_account_partner.status as saldostatus,
			a_customer_account_partner.invoice as invoicestatus,
			LEFT(pb.name , 3)  as partner_branch_name_short 
			FROM a_customer 
			...
			LEFT JOIN a_partner_branch as pb ON a_customer.p_b_account_created = pb.id
			LEFT JOIN a_customer_account_partner ON a_customer_account_partner.customer='".intval($cus_id)."' AND a_customer_account_partner.partner='".$current_partner."'
			WHERE  a_customer.id='".intval($cus_id)."'";
			 ...
			 $results = $query->row_array();
			  if ($query->num_rows() > 0) 
			  {
					$autofil_invoice=self::getAutofileamt($results['id']);
					if($autofil_invoice != '')
					{
						$results['autofil_invoice']=$autofil_invoice;
					}
					return $results;
			  } else {
					return false;
	...
	function updateAddress($street_line_1,$street_line_2,$floor,$calling_bell,$zip,$cid)
	{
		...
		{
			$sql="SELECT * FROM a_address WHERE customer='".$cid."' AND street_line_1='".$street_line_1."'$addsql1";
			$query=$this->db->query($sql);
			$status=$query->num_rows();
			if(intval($status) > 0)
			{
				...
				$qry="UPDATE a_address SET 
				street_line_2='".$street_line_2."',
				floor='".mysqli_real_escape_string($floor)."',
				calling_bell='".mysqli_real_escape_string($calling_bell)."',
				main='1'
				WHERE customer='".$cid."' AND street_line_1='".$street_line_1."'$addsql1";
				$this->db->query($qry);
			}
			else
			{
	...
	function customermobileinfo($mobile)
	{
		...
		$query=$this->db->query("SELECT b.firstname,b.lastname,b.sex,c.email,a.number as mobile,a.customer as id,d.street_line_1,d.street_line_2,d.floor,d.calling_bell,d.zip,p.name as partner_name,pb.name as partner_branch_name,a_customer_account_partner.autofil_invoice,LEFT(pb.name , 3)  as partner_branch_name_short 
		FROM a_phone a 
		...
		LEFT JOIN  a_customer_account_partner ON  b.id=  a_customer_account_partner.customer AND a_customer_account_partner.partner = '".$current_partner."'
		...
		if(intval($total) > 0)
		{
			...
			$autofil_invoice=self::getAutofileamt($response['id']);
			if($autofil_invoice != '')
			{
				$response['autofil_invoice']=$autofil_invoice;
			}
			return $response;
	...
	function companycustomermobileinfo($mobile)
	{
		...
		$query=$this->db->query("SELECT b.firstname,b.lastname,b.sex,c.email,a.number as mobile,a.customer as id,d.street_line_1,d.street_line_2,d.floor,d.calling_bell,d.zip,p.name as partner_name,pb.name as partner_branch_name,a_company_customer.type,a_company_customer.company,a_company.name as companyname,a_customer_account_partner.autofil_invoice,LEFT(pb.name , 3)  as partner_branch_name_short 
		FROM a_phone a 
		LEFT JOIN a_customer as b ON a.customer=b.id
		...
		LEFT JOIN a_email as c ON c.customer=a.customer
		LEFT JOIN a_address as d ON d.customer=a.customer
		LEFT JOIN  a_customer_account_partner ON  b.id=  a_customer_account_partner.customer AND a_customer_account_partner.partner = b.partner_branch
	...
	function customerAccountinfo($id)
	{
		
		$current_partner=$this->session->userdata['partner'];
		$current_partner_branch=$this->session->userdata['partner_branch'];
		
		$query=$this->db->query("SELECT b.id,b.firstname,b.lastname,b.sex,c.email,a.number as mobile,d.street_line_1,d.street_line_2,d.floor,d.calling_bell,d.zip,p.name as partner_name,pb.name as partner_branch_name,a_company_customer.type,a_company_customer.company,a_company.name as companyname,LEFT(pb.name , 3)  as partner_branch_name_short,a_customer_account_partner.autofil_invoice 
		FROM a_customer as b 
		LEFT JOIN a_phone as a ON a.customer=b.id
		LEFT JOIN a_company_customer ON a_company_customer.customer=b.id AND a_company_customer.customer=b.id AND a_company_customer.status='1'
		LEFT JOIN a_company ON a_company.id=a_company_customer.company
		LEFT JOIN a_partner_branch as pb ON pb.id=b.p_b_account_created
		LEFT JOIN a_partner as p ON p.id = pb.partner
		LEFT JOIN a_email as c ON c.customer=b.id
		LEFT JOIN a_address as d ON d.customer=b.id
		LEFT JOIN  a_customer_account_partner ON  b.id=  a_customer_account_partner.customer AND a_customer_account_partner.partner = '".$current_partner."'
		WHERE b.id='".trim($id)."'"); 
		$total=$query->num_rows();
		$response='';
	...
	function updateMobile($mobile,$cid)
	{
		...
		$sql="SELECT * FROM a_phone WHERE customer='".$cid."'";
		$query=$this->db->query($sql);
		$status=$query->num_rows();
		
		if(intval($status) == 0)
		{
			$qry="INSERT a_phone SET 
			customer='".$cid."',
			type='".$phone_type."',
			number='".$mobile."',
			main='1'";
		...
		else
		{
			...
			$sql="SELECT * FROM a_phone WHERE number='0' AND customer='".$cid."'";
			$query=$this->db->query($sql);
			$status=$query->num_rows();
			if(intval($status) > 0)
			{
				   $qry="UPDATE a_phone SET 
					main='0'
					WHERE customer='".$cid."'";
					$this->db->query($qry);
				   ...
				   $qry="UPDATE a_phone SET 
					main='1',
					number='".$mobile."'
					WHERE customer='".$cid."' AND number='0'";
					$this->db->query($qry);
			}
			else
			{
				$sql="SELECT * FROM a_phone WHERE number='".$mobile."' AND customer='".$cid."'";
				$query=$this->db->query($sql);
				$status=$query->num_rows();
				if(intval($status) > 0)
				{
					...
					$qry="UPDATE a_phone SET 
					main='1'
					WHERE customer='".$cid."' AND number='".$mobile."'";
					$this->db->query($qry);
				}
				else
				{
	...
	function getSaldoDetails($id)
	{
        ...
        if (! is_numeric($id)) {
		  ...
		  a_customer_account_log.id as salodid,
			  a_customer_account_log.customer,
			  a_customer_account_log.amount,
			  a_customer_account_log.type,
			  a_customer_account_log.partner,
			  a_customer_account_log.in_type,
			  a_customer_account_log.in_status,
			  a_customer_account_log.gift_card,
			  a_customer_account_log.employee,
			  DATE_FORMAT(regtime,'%d.%m.%Y %H:%i') as regtime,
			  CONCAT(IFNULL(a_customer.lastname,''),', ',IFNULL(a_customer.firstname,'')) AS customer_name,a_address.street_line_1,a_address.street_line_2,a_address.floor,a_address.calling_bell,a_city.city as city,a_address.zip as zip,a_phone.number,a_email.email,a_customer.id as customerid,a_zip.zone as zone
		  							FROM a_customer_account_log
                                    LEFT JOIN a_customer ON a_customer.id = a_customer_account_log.customer 
									...
									WHERE a_customer_account_log.id = $id ");
	...
	function getAutofileamt($customer)
	{
		$current_partner = $this->session->userdata['partner'];
		$current_partner_branch=$this->session->userdata['partner_branch'];
		$conditional_sql.=" AND partner = '".$current_partner."'";
		//$conditional_sql.=" AND partner_branch = '".$current_partner_branch."'";
		
		$query=$this->db->query("SELECT autofil_invoice FROM a_customer_account_partner WHERE customer='".trim($customer)."' $conditional_sql"); 
		$total=$query->num_rows();
		$amount='';
		if(intval($total) > 0)
		{
			$response=$query->row_array();
			if($response['autofil_invoice'] != '')
			{
				$amount=intval($response['autofil_invoice']);
			}
		}
	...
	function updateSaldostatus($customer,$status)
	{
		$current_partner=$this->session->userdata['partner'];
		$qry="UPDATE a_customer_account_partner SET status='".$status."' WHERE customer='".$customer."' AND partner ='".$current_partner."'";
		$this->db->query($qry);
		return true;
	}
	
	function updateInvoiceStatus($customer,$status)
	{
		$current_partner = $this->session->userdata['partner'];
		$current_partner_branch=$this->session->userdata['partner_branch'];
		$conditional_sql.=" AND partner='".$current_partner."' AND partner_branch = '".$current_partner_branch."'";
		
		$qry="UPDATE a_customer_account_partner SET invoice='".$status."' WHERE customer='".$customer."' $conditional_sql";
		$this->db->query($qry);
		return true;
	}
     ...
     * @return	array
     */
    function searchCompany($keyword='',$orderby = 'id', $sort = 'ASC',$type ='name')
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
		$current_partner_branch = $this->session->userdata['partner_branch'];
