...
// -- getVoucherType ----------------------------------------------------------------------------------------------
 ...
 /**
    ...
    function getSalesInfo($date='')
    {
        ...
        $this->debug_methods_trail[] = __function__;
        //declare
        $conditional_sql = " AND date='".$date."' AND partner_branch='".$this->session->userdata['partner_branch']."'";
	...
	function updateCashReport($casharray)
	{
	
		$date = date('Y-m-d',strtotime($this->input->post('from')));
		$query = $this->db->query("SELECT *  FROM a_daily_cash_report  WHERE `date`='".$date."'AND partner_branch='".$this->session->userdata['partner_branch']."'");
	...
	function dailyReport($fromdate,$todate)
	{
	...
	{
		$query = $this->db->query("SELECT * FROM a_opening_hours WHERE `special_day`='".$date."' AND partner_branch='".$this->session->userdata['partner_branch']."' AND status='1'");
		if($query->num_rows() > 0)
		{
			//$next_date = date('Y-m-d', strtotime($date . ' +1 day'));
			//$status=self::checkSpecialdate($next_date);
			//if($status)
			//{
				//return $next_date;
			//}
	...
	function getMonthName($month){
		$montharray=array('01'=>'Januar','02'=>'Februar','03'=>'Mars','04'=>'April','05'=>'Mai','06'=>'Juni','07'=>'Juli','08'=>'August','09'=>'September','10'=>'Oktober','11'=>'November','12'=>'Desember');
		return $montharray[$month];

	}
	
	
	/*check a value is found in an array*/
