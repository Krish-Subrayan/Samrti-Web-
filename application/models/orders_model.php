...
class Orders_model extends Super_Model{
    ...
    function __construct()
    {
		...
		/* Get customer order history*/
		function getCustomerOrderhistory($customer,$type ='all',$infotype='',$year='',$flag='')
		{
			...
			if($flag == 'pending')
			{
			$subaddsql=" AND DATE(a_orderline.p_b_delivery_time) < '".date('Y-m-d')."'";
			}
			...
			if(intval($year) > 0)
			{
				if($year == 1)
				{
					...
					$conditional_sql .=" AND a.id IN (select a_order.id from a_order,
a_orderline where 
a_order.id=a_orderline.order AND a_order.customer='".$customer."' AND 
(a_orderline.p_b_delivery_time IS NULL $subaddsql) GROUP BY `order` )";
				}
				else if($year == 2)
				{
					$conditional_sql .=" AND YEAR(a.order_time) >= '".date('Y')."' AND a.id IN (select a_order.id from a_order,
a_orderline where 
a_order.id=a_orderline.order AND a_order.customer='".$customer."' AND 
(a_orderline.p_b_delivery_time IS NOT NULL $subaddsql)GROUP BY `order` )";
...
(a_orderline.p_b_delivery_time IS NOT NULL $subaddsql) GROUP BY `order` )";
	  ...
	  function getCustomerDetails($id='')
	  {
        ...
        if (! is_numeric($id)) {
		  ...
		  $query = $this->db->query("SELECT a_order.customer,a_order.id,a_order.delivery_note,DATE_FORMAT(order_time,'%d.%m.%Y') as odate,DATE_FORMAT(order_time,'%H:%i') as otime,DATE_FORMAT(delivery_time,'%H:%i') as dtime,a_address.street_line_1,a_address.street_line_2,a_address.floor,a_address.calling_bell,a_city.city as city,a_address.zip as zip,a_phone.number,a_email.email,a_customer.id as customerid,a_zip.zone as zone,CONCAT(IFNULL(a_customer.lastname,''),', ',IFNULL(a_customer.firstname,'')) AS customer_name,a_customer_account_partner.autofil_invoice,a_customer.lastname as clname, a_customer.firstname as cfname
		  							FROM a_order
									...
									LEFT JOIN  a_customer_account_partner ON  a_order.customer=  a_customer_account_partner.customer AND a_customer_account_partner.partner = '".$current_partner."'
									WHERE a_order.id = $id ");
...
ON a_order.id=a_order_log.order WHERE a_order.id='".$id."' order by a_order_log.status desc limit 0, 1");*/
	...
	$query = $this->db->query(" SELECT a_order_log.status as order_status,a_order.*,total_amount,changed_amount,DATE_FORMAT(a_order.order_time,'%d.%m.%Y %H:%i:%s') as order_time,DATE_FORMAT(a_order_log.regtime,'%d.%m.%Y %H:%i:%s') as deliverytime,a_order_log.employee_p_branch
		FROM a_order 
		LEFT JOIN a_order_log
ON a_order.id=a_order_log.order WHERE a_order.id='".$id."' $conditional_sql order by a_order_log.status desc limit 0, 1");
    ...
    function getOrderLine($id='',$type='')
    {
		...
		if($type=='print'){
			$conditional_sql .= " AND a_orderline.payment_status !='canceled'";
		}
		
		if($type=='pending'){
			$conditional_sql .= " AND a_orderline.payment_status ='pending' AND a_orderline.payment_type ='account'";
		}
	...
	END AS oprice";
			...
			if(intval($current_partner_branch) == 14  || $current_partner_branch == 28 || $this->session->userdata['company_status']) //hvittsnip branch  ( only this branch aceept orders from comapny)
			{
				$customertype=$this->session->userdata['companytype'];
				$customercompany=$this->session->userdata['company'];
				$additional_sql.=" LEFT JOIN a_company_price ON a_company_price.type='".$customertype."' AND a_company_price.company='".intval($customercompany)."' AND a_company_price.product=a_product.id AND a_company_price.status='1'";
       ...
       $query = $this->db->query("SELECT *,a_orderline.price as price,a_orderline.special_instruction as description,a_orderline.id as id,DATE_FORMAT(a_orderline.p_b_delivery_time,'%d.%m.%Y') as utlevering,CONCAT('".PATH_IMAGE_FOLDER."',a_images.path) as path, a_product.type as ptype,a_product.name,a_orderline.product,a_product.price as oprice
		...
		WHERE 1=1 $conditional_sql AND a_orderline.order='".$id."' ORDER BY a_orderline.p_b_delivery_time ASC");
		
		}else{
			
        $query = $this->db->query("SELECT *,a_orderline.price as price,a_orderline.special_instruction as description,a_orderline.id as id,DATE_FORMAT(a_orderline.p_b_delivery_time,'%d.%m.%Y') as utlevering,CONCAT('".PATH_IMAGE_FOLDER."',a_images.path) as path, a_product.type as ptype,a_product.name,a_orderline.product,c.name as category,c.description as cdescription,a_orderline.payment_status as payment_status,
	$pricesql
		...
		LEFT JOIN a_images ON a_images.product=a_product.id 
		$additional_sql
		WHERE 1=1 $conditional_sql AND pbc.product_p_branch = a_orderline.product  AND  pbc.category_partner=c.id AND  pbc.status='1' AND  a_orderline.order='".$id."' GROUP BY a_orderline.id ORDER BY a_orderline.p_b_delivery_time ASC");
		}
	...
	function getProductDisplayName($id='')
	{
	  ...
	  WHEN pbc.display_name ='' THEN a_product.name 
	  ELSE pbc.display_name
	  END  AS name 
	  FROM a_product , a_product_p_branch_category as pbc
	  WHERE  a_product.id = pbc.product_p_branch AND pbc.product_p_branch = $id ORDER BY pbc.parent DESC");
	...
	//get pending orderlines
	function getPendingOrderlines($id='')
	{
	  ...
	  FROM a_orderline
	  WHERE  payment_status='pending' AND payment_type='account' AND a_orderline.order ='$id'");
	...
	{
		//$type='laundry';
		$query=$this->db->query("SELECT `id` FROM a_bag WHERE bag_barcode='".$barcode."'");
		if($query->num_rows() > 0)
		{
			$result=$query->row_array();
			return $result;
		...
		if($query->num_rows() == 0)
		{
			...
			if($order_status == 9)
			{
				...
				{	
					...
					{
						if(intval($orderitems['logid']) > 0)
						{
							$heatlogid=$orderitems['logid'];
							$qry=$this->db->query("SELECT id,orderline,company,customer,employee_p_branch,heat_seal FROM a_heat_seal_log WHERE id='".$heatlogid."' ORDER BY id DESC LIMIT 0,1");
							$log=$qry->row_array();
							if($qry->num_rows() > 0)
							{
								$addsql=""; 
								if($this->session->userdata('employee_p_branch') != '')
								{
									$addsql.=",employee_p_branch='".$this->session->userdata('employee_p_branch')."'";
								}
								...
								if($orderitems['customer'] != '')
								{
									$addsql.= ",customer='".$orderitems['customer']."'";
								}
								//finished
								$qry="INSERT INTO a_heat_seal_log SET `heat_seal`='".$log['heat_seal']."',`orderline`='".$log['orderline']."',`status`='18',`regtime`='".date('Y-m-d H:i:s')."'$addsql";	
								$query=$this->db->query($qry);	
			...
			if(!empty($data))
			{
				...
				for($j=0;$j<count($data);$j++)
				{
					...
					if($orderline == $data[$j]['id']){
						...
						if(intval($changed_quantity) > 0) 
						{
							$subqry="UPDATE a_orderline SET payment_status='pending' WHERE id='".$orderline."' AND `order`='".$orderid."' AND payment_status='canceled'";
							$this->db->query($subqry);
						}
	...
	function saveOrder($request,$payment_status)
	{
			...
			$delivery_note=mysqli_real_escape_string($request["delivery_note"]);
			$special_instruction=mysqli_real_escape_string($request["special_instruction"]);
			...
			$query=$this->db->query("SELECT * FROM a_partner_branch WHERE id='".$current_partner_branch."' AND partner='".$current_partner."'");
			$partners=$query->row_array();
			$order_id_start=intval($partners['order_id_start']);
			$min=intval($partners['order_id_low']);
			$max=intval($partners['order_id_high']);
			$next_order_id_start= $order_id_start + 1 ;
			
			$bool = ($min <= $order_id_start) && ($order_id_start <= $max) ?  true : false ;
			...
			$mva=$total_amount/1.25;	
			$mva=round($mva, 2);
			$mva=$total_amount-$mva;
				...
				foreach($products as $cartitem)
				{
					...
					if($this->session->userdata['partner_branch'] == 14 || $this->session->userdata['partner_branch'] == 28 || $this->session->userdata['company_status'])
					{
						...
						if($proqty == 1)
						{
							...
							$heat_seal  = $cartitem['options']['heatseal'][0]['barcode'];
							...
							if($heat_seal != '')
							{
								...
								$query=$this->db->query("SELECT * FROM a_heat_seal WHERE heat_seal_barcode='".$heat_seal."' ORDER BY id DESC LIMIT 0,1");	
								...
								//added
								$qry="INSERT INTO a_heat_seal_log SET `heat_seal`='".$heatsealid."',`orderline`='".$orderline."',`status`='1',`regtime`='".date('Y-m-d H:i:s')."',employee_p_branch='".$this->session->userdata('employee_p_branch')."',customer='".$customer."',company='".$this->session->userdata['customer']['company']."'";	
						...
						{
							$hs_arr = array_values($cartitem['options']['heatseal']);
							
							if(count($hs_arr) > 0)
							{
								foreach($hs_arr as $heatseal)
								{
										...
										if($heatseal['barcode'] != '')
										{
											...
											$query=$this->db->query("SELECT * FROM a_heat_seal WHERE heat_seal_barcode='".$heatseal['barcode']."' ORDER BY id DESC LIMIT 0,1");		
											...
											{
												$qry="INSERT INTO a_heat_seal SET 
												`heat_seal_barcode`='".$heatseal['barcode']."',
												product='".$cartitem['id']."',
												additional_product='".$heatseal['type']."',
												`orderline`='".$orderline."',
												`regtime`='".date('Y-m-d H:i:s')."'";
												$query=$this->db->query($qry);	
												$heatsealid=$this->db->insert_id();
											...
											//added
											$qry="INSERT INTO a_heat_seal_log SET `heat_seal`='".$heatsealid."',`orderline`='".$orderline."',`status`='1',`regtime`='".date('Y-m-d H:i:s')."',employee_p_branch='".$this->session->userdata('employee_p_branch')."',customer='".$customer."',company='".$this->session->userdata['company']."'";	
			...
			if($boolean){	
				...
				$qry="INSERT INTO a_order_log 
						...
						partner_branch='".$current_partner_branch."',
						employee_p_branch='".$this->session->userdata['employee_p_branch']."',
						status='5'";  // order scanned . Order placed by heatseal
						$query=$this->db->query($qry);
			}
			...
			$sql="UPDATE a_partner_branch SET order_id_start='".$next_order_id_start."'  WHERE id='".$current_partner_branch."' AND partner='".$current_partner."'";
			$this->db->query($sql);
...
//send email now
 ...
 $query = $this->db->query("INSERT INTO email_log (
	...
	function orderlinePayment($orderid,$orderline,$payment_type,$payment_status,$totalorderline,$pending)
	{	
		
		$addsql =" AND payment_status='pending' AND payment_type='account'";
		
		if(trim($payment_type) == 'invoice')
		{
			$payment_status='pending';
		}
		...
		if($totalorderline == count($orderlines)){
			...
			self::updateorderlog($orderid,9);//For hide non saldo customer paid for order placed time.
			
		}
		else if($totalorderline == $pending){
			...
			//For hide non saldo customer paid for order placed time.
	...
	function orderlineHeatseal($id='')
	{
        ...
        $this->benchmark->mark('code_start');
		//_____SQL QUERY_______ 
		//a_heat_seal_log.customer,
        $query = $this->db->query("SELECT a_product.name,a_orderline.id as id,a_orderline.amount,a_order.customer,a_orderline.changed_amount,DATE_FORMAT(a_orderline.p_b_delivery_time,'%d.%m.%Y') as utlevering,a_product.description,a_product.type as ptype,a_heat_seal.heat_seal_barcode as barcode,a_heat_seal.id as heatid,a_heat_seal.additional_product,a_heat_seal_log.status,a_heat_seal_log.id as logid
		FROM a_orderline 
		...
		WHERE a_orderline.id='".$id."' AND a_heat_seal_log.id in (SELECT MAX(id) FROM `a_heat_seal_log` WHERE 1=1 AND a_orderline.id=a_heat_seal_log.orderline GROUP BY `heat_seal`) ");
	...
	/* Get customer order history*/
		function getInfileOrders($date='')
		{
			...
			*/
			...
			if($date != '')
			{
				$addsql=" AND DATE(regtime)='".$date."'";
			}
			$qry="SELECT `order` FROM a_orderline WHERE id IN (SELECT orderline FROM a_heat_seal_log WHERE id IN(SELECT max(id) as id FROM a_heat_seal_log GROUP BY `orderline`) $addsql AND status='2') GROUP BY `order`";
			...
			$add_sql
			WHERE 1=1
			$conditional_sql ORDER BY a.order_time ASC";
  ...
  //profiling::
	...
	END AS oprice";
			...
			if(intval($current_partner_branch) == 14 || intval($current_partner_branch) == 28 || $this->session->userdata['company_status']) //hvittsnip branch  ( only this branch aceept orders from comapny)
			{
				$customertype=$this->session->userdata['companytype'];
				$customercompany=$this->session->userdata['company'];
				$additional_sql.=" LEFT JOIN a_company_price ON a_company_price.type='".$customertype."' AND a_company_price.company='".intval($customercompany)."' AND a_company_price.product=a_product.id AND a_company_price.status='1'";
	...
	function updateHeatsealstatus($orderlineheatdata,$status)
	{
	...
	function updateOrderlinelogstatus($orderid,$orderline)
	{
	

				$orderlines=$this->process_order_model->getOrderLine($orderid,$orderline);	
    ...
    function getOrderLinecategory($id='')
    {
		...
		$query = $this->db->query("SELECT c.id as catid
	
	    FROM a_product_p_branch_category as pbc,a_category_partner as c,a_orderline 
	
		WHERE pbc.product_p_branch = a_orderline.product  AND  pbc.category_partner=c.id AND  pbc.status='1' AND a_orderline.order='".$id."' GROUP BY a_orderline.id ORDER BY a_orderline.p_b_delivery_time ASC");
		...
		if(count($results) > 0)
		{
			foreach($results as $catres)
			{
				$catarray[$catres['catid']]=$catres['catid'];				
			}
		}
	...
	/*update orderline log status*/
	function updateOrderlinelog($orderline,$status)
	{
	
		$query=$this->db->query("SELECT * FROM a_order_log WHERE `orderline`='".$orderline."' AND status='".$status."'");
