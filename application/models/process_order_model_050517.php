...
a_order.order_time,a_order.type,a_order.changed_amount,a_order.total_amount,pb.name as partner_branch
	...
	function createInfiledownload()
	{
		...
		LEFT JOIN a_order_log ON a_order.id = a_order_log.order
		WHERE a_order.type='shop' AND (a_order_log.id) in (SELECT id FROM a_order_log WHERE id IN(SELECT max(id) as id FROM a_order_log GROUP BY `order`) AND `status`= 5 ORDER by id DESC) $addsql ORDER BY a_order.id LIMIT 0,10";
	...
	LEFT JOIN a_partner_branch as pb ON a_order.partner_branch=pb.id
	LEFT JOIN a_partner as p ON pb.partner=p.id
	WHERE a_order.type='shop' AND a_heat_seal_log.id in (SELECT id FROM a_heat_seal_log WHERE id IN(SELECT max(id) as id FROM a_heat_seal_log GROUP BY `heat_seal`) AND `status`= 2 ORDER by id DESC) $addsql ORDER BY a_order.id";
