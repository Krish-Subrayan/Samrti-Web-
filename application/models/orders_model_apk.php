...
class Orders_model extends Super_Model{
    ...
    function __construct()
    {
		...
		/* Get customer order history*/
		function getCustomerOrderhistory($customer,$type ='all',$infotype='',$year='')
		{
			$add_sql='';
			$conditional_sql ="";
...
a_orderline.p_b_delivery_time IS NULL GROUP BY `order` )";
				}
				else if($year == 2)
				{
...
a_orderline where 
a_order.id=a_orderline.order AND a_order.customer='".$customer."' AND 
a_orderline.p_b_delivery_time IS NOT NULL GROUP BY `order` )";
