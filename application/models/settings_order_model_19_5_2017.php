				...
				if(count($orderdetails) > 0)
				{
				   ...
				   //update payment status
					if($orderinfo['payment_status'] == 'paid' )
					{
						...
						$result = $this->payments_model->updateCustomerBalance($customer,$totalamount,$in_status,'credit',$saldo_status);
					...
					if($query->num_rows() > 0)
					{
						...
						if($result['status'] == 'pending')
						{
							...
							if($orderline_arr['payment_status'] == 'paid')
							{
								...
								$saldo_status = $this->payments_model->getSaldostatus($customer); //check is the customer having saldo 
								$result = $this->payments_model->updateCustomerBalance($customer,$amount,$in_status,'credit',$saldo_status);
