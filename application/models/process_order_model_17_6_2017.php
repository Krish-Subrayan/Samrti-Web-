...
JOIN a_collection_time AS ct2 ON a_order.delivery_time = ct2.id
	...
	{
		$query=$this->db->query("SELECT * FROM a_heat_seal_log WHERE orderline='".$orderline."' AND status='2'");		//started
...
a_order.order_time,a_order.type,a_order.changed_amount,a_order.total_amount,pb.name as partner_branch
	...
	//a_product_additional_product.additional_product
			$sql="SELECT a_product_additional_product.id as id,a_additional_product.name FROM 
	...
	function createInfiledownload()
	{
		...
		if(count($orderlines) > 0)
		{
			...
			{
				$scanqty = $this->process_order_model->getHeatOrderLine($oritems['orderline']);
	...
	LEFT JOIN a_partner_branch as pb ON a_order.partner_branch=pb.id
	LEFT JOIN a_partner as p ON pb.partner=p.id
	WHERE a_order.type='shop' AND a_order.in_status='0' AND a_heat_seal_log.id in (SELECT id FROM a_heat_seal_log WHERE id IN(SELECT max(id) as id FROM a_heat_seal_log GROUP BY `heat_seal`) AND `status`= 2 ORDER by id DESC) $addsql ORDER BY a_order.id";
		...
		{	
			...
			foreach($orderlines as $orderitems)
			{
				...
				//$total_items =$orderitems['quantity'];
				...
				$qry="INSERT INTO a_heat_seal_log SET `heat_seal`='".$orderitems['heat_seal']."',`orderline`='".$orderitems['orderline']."',`status`='5',`regtime`='".date('Y-m-d H:i:s')."',customer='".$orderitems['customer']."',employee_p_branch='".$orderitems['employee_p_branch']."'$addsql";	
				$query=$this->db->query($qry);	*/
				...
				$indata.=$orderid.$garcode.$garment_name.$garment_per.$hung.$garment_status;*/
	...
	$query=$this->db->query("SELECT a_order.id AS orderid,a_order.order_time,a_customer.firstname,a_customer.lastname,CONCAT(IFNULL(a_customer.firstname,''),', ',IFNULL(a_customer.lastname,'')) AS customer_name,a_orderline.regtime,a_heat_seal.heat_seal_barcode, a_phone.number as phone,a_product.name, CASE WHEN a_orderline.quantity IS NULL THEN a_orderline.quantity ELSE a_orderline.quantity END AS quantity, a_orderline.id as orderline,DATE_FORMAT(order_time,'%d.%m.%Y') as odate,DATE_FORMAT(order_time,'%H:%i') as otime,DATE_FORMAT(ctime.start_time,'%d/%m/%y') as collection_time,DATE_FORMAT(dtime.start_time,'%d/%m/%y') as delivery_time ,a_address.zip,a_orderline.p_b_delivery_time FROM a_order 
LEFT JOIN a_customer ON a_customer.id=a_order.customer 
LEFT JOIN a_phone ON a_phone.customer=a_customer.id 
