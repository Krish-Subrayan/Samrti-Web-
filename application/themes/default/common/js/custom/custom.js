...
//--REFRESH PARENT ON CLOSING BOOTSTRAP MODAL--------------------------------------------------------------------------------------------------
$(document).ready(function() {
	
	$('#cancel-modal-action').click(function() {
		console.log('foo');
		$('#modalIframe').removeClass('reload-parent-on-close');
		$('#modalIframe').attr('data-reload-window', 'no');
	})

	$('.reload-parent-on-close').on('hidden.bs.modal', function() {
		//do not reload if we have pressed the cancel button
		if ($(this).attr('data-reload-window') != 'no') {
			location.reload();
		} else {
			//reset for next time
			$(this).attr('data-reload-window', '');
		}
	});
});
...
/** add custom css to "messages content" that was created by CKEditor
 *  - make images responsive
 *  - make tables look like default bootstrap tables
 */
$(document).ready(function() {

	//make images responsive
	$('.messaging-text').children("img").css("width", "");
	$('.messaging-text').children("img").css("height", "");
	$('.messaging-text').children("img").removeAttr('width').removeAttr('height').addClass("img-responsive");

	//make tables bootstrap styled
	$('.messaging-text').children("table").addClass("table").addClass("table-bordered").addClass("table-bordered-outlined");
});

/**
 *icheck.js pretty check boxes
 */
$(document).ready(function() {
	$('.icheckjs').iCheck({
