...
class Printecspos extends MY_Controller
{
				...
				$this->data['debug'][] = $this->customer_model->debug_data;
					...
					//set invoice name
					 if (!empty($this->data['blocks']['account'])) {
						...
						$cmds .= '  kr '.formatcurrency($this->data['blocks']['account'][$i]['amount']);
						
						$cmds .= $newLine;
						
						$cmds .= str_replace($search, $replace,$this->data['blocks']['account'][$i]['customername']);
				...
				if($olineitmes['product'] !=1){  //exclde Skjorte bec'z Skjorte has only heatseal not tag
				 ...
				 $prodtype=$this->process_order_model->validateProducttype($olineitmes['product']);
				  ...
				  else
				  {
					...
					$qrynamearray=array();
					if(intval($prodtype) > 1)
					{
						$proudcttypes=$this->process_order_model->getproducttypes($olineitmes['id']);
						$pi=0;
						$newprotype=$prodtype-1;
						foreach(range(1,$qty) as $qtys)
						{ 
							
							$qrynamearray[$qtys]=$proudcttypes[$pi]['name'];
							if($pi == $newprotype)
							{
								$pi=0;
							}
					...
					if($qty > 0)
					{
						...
						{ 
							...
							if(intval($prodtype) > 1)
							{
								$cmds .= str_replace($search,$replace, $qrynamearray[$qtys]);
							}
							else
							{
								$cmds .= str_replace($search,$replace, $olineitmes['name']);
							}
	...
	echo '<script>
		...
		//$webClientPrintControllerAbsoluteURL = APPPATH . 'third_party/WebClientPrint/WebClientPrintController.php';
		...
		//$demoPrintFileProcessAbsoluteURL =  APPPATH . 'third_party/WebClientPrint/WebClientPrintCommandsProcess.php?order='.$orderid.'&printtype='.$printtype.'&tag_printer='.$this->session->userdata['tag_printer'].'&bill_printer='.$this->session->userdata['bil_printer'];
