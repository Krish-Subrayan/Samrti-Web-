...
class Printecspos extends MY_Controller
{
					  ...
					  else
					  {
						...
						if($qty > 0)
						{
							foreach(range(1,$qty) as $qtys)
							{
								...
								$cmds .= $esc . '!' . '0x38'; //Emphasized + Double-height + Double-width mode selected (ESC ! (8 + 16 + 32)) 56 dec => 38 hex
								$cmds .= $orderid.'            ';
								$cmds .= $esc . '24' . '0x38'; //Emphasized + Double-height + Double-width mode selected (ESC ! (8 + 16 + 32)) 56 dec => 38 hex
								
								$cmds .= $esc . '!' . '0x10'; //Character font A selected (ESC ! 0)
								$cmds .= $esc . 'a' . '0x02';
								$cmds .= $orderid.' '.str_replace($search, $replace, $companyname).' '.$partnerinfo['id'].' ';								$cmds .= $newLine;
								$cmds .= str_replace($search, $replace,$customer['customer_name']);
								$cmds .= $newLine;		
								...
								$cmds .= $esc . 'a' . '0x02';
								$cmds .= $esc . '!' . '0x10'; //Character font A selected (ESC ! 0)
								$cmds .= ' I '.date('d/m',strtotime($orderinfo['order_time']));
								$cmds .= $newLine;
								$cmds .='Rdy '.$ready;
								$cmds .= $newLine;
								$cmds .= $esc . '!' . '0x38';
								$cmds .= $esc . 'a' . '0x00';
								$cmds .= str_replace($search, $replace,$weekday).':'.$item.'/'.$total_qty;
								$cmds .= $esc . 'a' . '0x02';
								$cmds .= $esc . '!' . '0x10'; //Character font A selected (ESC ! 0)
								$cmds .= str_replace($search, $olineitmes['name']);
								$cmds .= $newLine;
								$cmds .= str_replace($search, $customer['number']);
								$cmds .= $newLine;
								$cmds.=$pagebreak;
								$item ++;
    ...
    function __printBill($printtype)
    {
       ...
       // $cmds .= $this->data['fields']['customer']['address'];
		...
		$printerName='EPSON TM-T88III Receipt';
		$printerNamer2 = 'EPSON TM-U220 Receipt';
		...
		echo '<input type="hidden" value="true" name="pid" id="pid"  value="0"/> ';
		echo '<input type="hidden" name="printerCommands" id="printerCommands" value="'.$filename.'" /> ';
		echo '<input type="hidden" name="printerCommands2" id="printerCommands2" value="'.$filename.'" /> ';
		
		echo '<a style="visibility:hidden;"  id="printbtn" class="btn btn-success btn-large" onclick="javascript:jsWebClientPrint.print(\'pid=\' + $(\'#pid\').attr(\'checked\') + \'&printerName='.$printerName.'& + \'&printerName2='.$printerName2.'&printerCommands=\' + $(\'#printerCommands\').val());">Print File...</a>';
