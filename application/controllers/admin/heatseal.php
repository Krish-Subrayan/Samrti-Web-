...
class Heatseal extends MY_Controller
{
		   ...
		   case 'updateMissStatus':
			...
			case 'upatedeliverystatus':
				 $this->__upatedeliverystatus();
           		 break;	
			case 'barcodeinfovelg':
				$this->__barcodeinfovelg();
              break;
			default:
             ...
             $company_info ='<div class="company-info">
				 ...
				 if($this->session->userdata['partner_branch'] != 1000)
				{
					//$company_info .='<a href="#editproduct" onclick="editOrder(\''.$order_id.'\');" id="editproductbtn" data-toggle="modal" class="btn-icon"><i class="fa fa-edit" aria-hidden="true"></i></a>';
				   			...
				   			$company_info .='<a href="javascript:void(0);" onclick="heatsealprint(\''.$order_id.'\');"  data-toggle="modal" class="btn-icon"><i class="fa fa-print" aria-hidden="true"></i></a>
			...
			for($i=0;$i< count($orderdetails);$i++)
			{
			 ...
			 /* $str.='<tr>
			  ...
			  $str.='<div style="cursor:pointer;" class="orderlist cart_table_item">
                   ...
                   <div class="pull-left col-md-3 no-padd text-right" ><p><b>kr '.formatcurrency($amount).$vary.'</b></p></div>
                   <div class="clearfix"></div>
                 </div>';
			...
			if(!empty($data)){
			 ...
			 $str='<div style="cursor:pointer;" class="orderlist cart_table_item">
                  ...
                  <p><b>Artikler</b></p>
                   ...
                   <div class="pull-left col-md-3 no-padd text-right" ><p><b>Beløp</b></p></div>
				   <div class="clearfix"></div>
                 <hr />
                 </div>';
				...
				if($customer)
				{
					...
					if(intval($mobileinfo['id']) > 0)
					{
						...
						if($ccstatus)
						{
							...
							$compnayinfo=$this->customer_model->getCompanyinfo($mobileinfo['company']);
							$company=$compnayinfo['id'];
						}
					...
					if(count($compnayinfo) > 0)
					{
						...
						$companyhtml='<div class="col-md-9 col-sm-6 col-xs-6 no-padd">';
						$companyhtml.='<h4 class="mb-none"><strong>'.$compnayinfo['name'].'</strong></h4>';
						$companyhtml.='<p style="color:#5a5a5a;">'.$compnayinfo['street'].'</p>';
						$companyhtml.='<p style="color:#5a5a5a;">'.$compnayinfo['phone'].'</p>';
						$companyhtml.='<p style="color:#5a5a5a;">'.$compnayinfo['email'].'</p>';
						$companyhtml.='<input type="hidden" id="company_id" name="company_id" value="'.$company.'" />';
						$companyhtml.='</div>';
					...
					else
					{
						...
						//onchange="saveCompany(this.value);"
						//$heatinfo='<div class="col-md-12 col-sm-6 col-xs-6 no-padd">';
						$companyhtml.='<select name="company" onchange="setCompany(this.value);"  id="company" class="selectbox"><option value="">-- Select Company --</option>';
							...
							if(count($companylist) > 0)
							{
								...
								{
									$companyhtml.='<option value="'.$list['id'].'">'.$list['name'].'</option>';
								}
							}
							$companyhtml.='</select>';
							//$heatinfo.='</div>';
                    ...
                    $producthtml.='<div class="col-md-3">
                          Product	:</div>
                     <div  class="col-md-6">
                      <p><span class="company_name"><strong>'.$productinfo['name'].'</strong></span></p>
                    </div>';
						
				
						/*$producthtml.='<div id="summary_1" class="summary entry-summary">
					...
					$result = array('mobile'=>$mobile,"status"=>'success','companyhtml'=>$companyhtml,'producthtml'=>$producthtml,"data"=>$heatinfo,'customerstatus'=>$customerstatus);
				}
			else
			{
				$barcode=$_POST['barcode'];
				...
				$customerstatus=0;
				$result = array('mobile'=>'',"status"=>'success','companyhtml'=>$companyhtml,'producthtml'=>'',"data"=>'','customerstatus'=>$customerstatus);
						...
						if($producttypes)
						{
							...
							foreach($producttypes as $proitems)
							{
							 ...
							 if(($i%3)==0 && ($i !=0 ))	
								...
								{
									$outhtml.=' <input disabled="disabled" type="radio" class="producttype" value="'.$proitems['id'].'" id="producttype_'.$proitems['id'].'" name="producttype" onclick="producttypepopup(\''.$proitems['name'].'\',\''.$proitems['id'].'\',\''.$product.'\');" />';
								}
								else
								{
									$outhtml.=' <input type="radio" class="producttype" value="'.$proitems['id'].'" id="producttype_'.$proitems['id'].'" name="producttype" onclick="producttypepopup(\''.$proitems['name'].'\',\''.$proitems['id'].'\',\''.$product.'\');" />';
								}
	...
	function __insertCart($productinfo,$barcode){
		  ...
		  if(count($productinfo) > 0)
		  {
			...
			if(is_array($barcode)){
				$this->session->userdata['heatsealcart'][] = array_values($barcode);
			}
			else{
				$arr = array($barcode);
				$this->session->userdata['heatsealcart'][] = $arr ;
			}
	...
	function __producttypecart()
	{
		...
		else
		{
			...
			$s_data['producttypecart']=$this->session->userdata['producttypecart'];
			//$s_data['producttypecart'][$product][$producttype]=$heatseal;
			$s_data['producttypecart'][$product][$producttype]['name']=$name;
			$s_data['producttypecart'][$product][$producttype]['type']=$producttype;
			$s_data['producttypecart'][$product][$producttype]['barcode']=$heatseal;
	...
	//echo '<pre>';print_r($this->session->userdata);exit;
		...
		//	echo $sqty.'=>'.$proqty;exit;
					if($proqty <= $sqty)
					{
						...
						if($productinfo)
						{
							self::__insertCart($productinfo,$s_data['producttypecart'][$product]);
						}
						else
						{
	...
	function __getProducttype()
	{
		...
		$producttypes=$this->process_order_model->get_product_types($product);
						...
						/*$outhtml.='<button type="button" data-dismiss="modal" onclick="addTocartDT();" class="btn green addProducbutton">Add to Cart</button>
                ...
                $outhtml.='<div class="clearfix"></div>
             </div>';
			...
			if(isset($_POST['barcode']))
			{
				...
				if($baginfo)
				{
					...
					else
					{
						...
						$message='The bag belongs to Smart Laundry.';
						$bagstatus = ($this->session->userdata['partner_branch']=='1000') ?  1  : 0;
						$result = array('status'=>'success','bagstatus'=>$bagstatus,'message'=>$message);
					}
				...
				{
					$message='The bag is not yet assinged to any customer.';
...
$lstatus++;
			 ...
			 $boolean = true;
			  foreach($barcodes as $baritems)
			  {
				...
				if($baritems['barcode'] != '')
				{
					...
					if($baritems['status'] == '17')
					{
							$str.='<input type="button" value="'.$this->data['lang']['lang_found'].'" class="orange" onclick="updateMissStatus(\''.$baritems['heatid'].'\',\'21\',\''.$baritems['id'].'\');" /></td></tr>';
	...
	function __updateMissStatus()
	{
		...
		{	
			...
			if($status == 17)
			{
				$buttonstr='<input type="button" value="'.$this->data['lang']['lang_found'].'" class="orange" onclick="updateMissStatus(\''.$heatsealid.'\',\'21\',\''.$orderline.'\');" />';
				$newstatus='21';
			}
			if($status == 21)
			{
	...
	function __deliveryOrder()
		...
		//echo '<pre>';print_r($orderinfo['id']);exit;
...
<input type="checkbox" onClick="paycheckall(this)" id="newcheckall"  />
</div>';
			...
			//	echo $data[$j]['id'];exit;
				...
				//	$total = ($delsum + $delivery_charge) - $discount ;
					...
					if(in_array($data[$j]['id'], $canceled_arr)){
						...
						$data[$j]['payment_status']  = ($data[$j]['payment_type'] == 'invoice' && $data[$j]['payment_status']='pending') ? 'paid' : $data[$j]['payment_status'];
						
					
					    if($chkboxstatus)
						{
						$str.=' <div id="tr_'.$data[$j]['id'].'" class="orderlist '. $cstatus.'">';
						}
						else
						{
					...
					if($data[$j]['payment_status'] == 'pending')
					{
						...
						{
							$str.= '<input type="checkbox" '.$disable.' id="corderlines_'.$data[$j]['id'].'"  value="" />';
						}
					...
					{	
						...
						{
							 $str.= '<input type="checkbox"  '.$disable.'  name="corderlines" id="corderlines_'.$data[$j]['id'].'"  value="" />';
						}
	...
	function __upatedeliverystatus()
	 {
		...
		$customer_id=$this->session->userdata['pos_customer_id'];
		$customer = $this->orders_model->getCustomerDetails($orderid);
		if($customer['customer'] != $customer_id)
		{	
			$result = array("status"=>'error',"message"=>"Order is not realated to this customer. Please contact our administrator");
			echo json_encode($result);exit;
		}
		...
		{	
			...
			{	
				...
				if(count($heatdata) > 0)
				{
					if($heatdata[0]['customer'] != $customer_id)
					{	
						$result = array("status"=>'error',"message"=>"Order is not realated to this customer. Please contact our administrator");
						echo json_encode($result);exit;
					}
				...
				if(count($orderdetails) != count($corderlines))
				{
	 ...
	 function __barcodeinfovelg()
	 {
			$heatinfo ='';
			$heatinfo.=' <input type="hidden" id="access_heatseal" name="access" value="[vars.access]"/>
			<form action="'.base_url().'admin/customer/company"  name="heatname_form" method="post" id="frm_cnavn_heatseal">
			<div class="clearfix">
			...
			<div class="col-xs-3">
			<input type="button" onclick="registerCustomername();" value="SÃ¸k" class="highlight btn btn-default pull-right">
			</div>
			</div>
			</form>

			<form action="'.base_url().'admin/customer/company" name="heatmoble_form" method="post" id="frm_cmobil_heatseal">
			<div class="clearfix">
			...
			<div class="col-xs-3">
			<input type="button" onclick="registerCustomermobile();" value="SÃ¸k" class="highlight btn btn-default pull-right">
			</div>
			</div>
			</form>';
