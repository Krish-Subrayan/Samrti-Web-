		...
		{	
			...
			{				
				...
				{					
					...
					$profiledata='<div class="pull-left col-md-4 no-padd text-center">
						 <img src="img/avatar.png" alt="" title="">
					   </div>
					   <div class="pull-left col-md-8 no-padd customer-info">
						   <p><span>'.$name.'</span>
						   (+47) '.$result['phone'].'<br>
			...
			if(intval($cstaff) > 0)
			{
				...
				//$newtime="starttime_".$cstaff."({precision: 'seconds', startValues: {".$totallogintime."}})";	
					...
					$staffprofile='<style>.employee-profile{display:block;}</style>
					   <div class="pull-left col-md-4 no-padd text-center">
		...
		if($loginstafflist)
		{
			...
			foreach($loginstafflist as $logitem)
			{
				...
				$staffhtml.='<div class="row  mt-sm">
				 ...
				 <div class="col-md-2"><p>'.$logitem['in'].'</p></div><div class="col-md-2"><p>';
				 if(strcmp($logitem['out'] ,'00:00'))
				 $staffhtml.=$logitem['out'];
				 $staffhtml.='</p></div>';
