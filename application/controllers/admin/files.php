...
class Files extends MY_Controller
{
    ...
    function __filesAdd()
    {
        ...
        if ($next) {
            ...
            if ($result = $this->files_model->addFile()) {
                ...
                //email - client
                $this->__emailer('mailqueue_new_file_client', $vars);
                /** --------------------------emailer - v2----------------------------------------------------------*/
    ...
    function __emailer($email = '', $vars = array())
    {
        ...
        //------------------------------------queue email in database-------------------------------
        /** THIS WIL NOT SEND BUT QUEUE THE EMAILS*/
        if ($email == 'mailqueue_new_file_client') {

            //get message template from database
            $template = $this->settings_emailtemplates_model->getEmailTemplate('new_projectfile_client');
            ...
            for ($i = 0; $i < count($this->data['vars']['project_mailing_list']); $i++) {
                ...
                if ($this->data['vars']['project_mailing_list'][$i]['user_type'] == 'client') {
                    ...
                    $vars['url_file'] = $this->data['vars']['site_url_client'] . '/files/' . $this->project_id . '/view';
                    $vars['email_signature'] = $this->data['settings_company']['company_email_signature'];
