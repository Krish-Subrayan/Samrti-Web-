...
class Products extends MY_Controller
{
			...
			if($this->session->userdata['current_heatseal'])
			{
				...
				if(intval($product) > 0)
				{
					...
					if($proqty == 1)
					{
						...
						self::__insert_Cart($productinfo,$barcode);
	...
	function __insertCart(){
		  ...
		  if(count($_POST) > 0)
		  {
			...
			$result = array("order_list_rens"=>$str_rens,"order_list_vask"=>$str_vask,"delsum"=>$total,"mva"=>$delsumvat,"count"=>$count,"delsum_currency"=>formatcurrency($total),"mva_currency"=>formatcurrency($delsumvat),'lastproduct'=>$this->session->userdata['lastproduct']);
			echo json_encode($result);exit;
			...
			for($i=0;$i<count($this->data['blocks']['products']);$i++){
				...
				if(($i%4)== 0){
				 ...
				 if($this->data['blocks']['products'][$i]['child'] > 0){
					...
					if($proqty == 1)
					{
						 $str .='<div class="pblock"  style="background:#'.$color.'"><a class="productimg" id="pro_'.$this->data['blocks']['products'][$i]['id'].'"  href="#" data-qty="'.$proqty.'" data-stuff="'.$this->data['blocks']['products'][$i]['id'].'@'.$this->data['blocks']['products'][$i]['name'].'@'.$this->data['blocks']['products'][$i]['price'].'@'.$this->data['blocks']['products'][$i]['path'].'@'.$this->data['blocks']['products'][$i]['description'].'@'.$this->data['blocks']['products'][$i]['gtype'].'@'.$this->data['blocks']['products'][$i]['duration'].'@'.$this->data['blocks']['products'][$i]['note'].'" class=""  data-title="'.$this->data['blocks']['products'][$i]['name'].'">'.$this->data['blocks']['products'][$i]['name'].'<span class="price">kr '.formatcurrency($this->data['blocks']['products'][$i]['price']).'</span></a></div>';
					}
					else
					{
						 $str .='<div class="pblock"  style="background:#'.$color.'"><a class="productimg" id="pro_'.$this->data['blocks']['products'][$i]['id'].'"  href="#" data-qty="'.$proqty.'" data-stuff="'.$this->data['blocks']['products'][$i]['id'].'@'.$this->data['blocks']['products'][$i]['name'].'@'.$this->data['blocks']['products'][$i]['price'].'@'.$this->data['blocks']['products'][$i]['path'].'@'.$this->data['blocks']['products'][$i]['description'].'@'.$this->data['blocks']['products'][$i]['gtype'].'@'.$this->data['blocks']['products'][$i]['duration'].'@'.$this->data['blocks']['products'][$i]['note'].'" class=""  data-title="'.$this->data['blocks']['products'][$i]['name'].'">'.$this->data['blocks']['products'][$i]['name'].'<span class="price">kr '.formatcurrency($this->data['blocks']['products'][$i]['price']).'</span></a></div>';
