             ...
             $company_info ='<div class="company-info">
				 ...
				 if($this->session->userdata['partner_branch'] != 1000)
				{
					$company_info .='<a href="#editproduct" onclick="editOrder(\''.$order_id.'\');" id="editproductbtn" data-toggle="modal" class="btn-icon"><i class="fa fa-edit" aria-hidden="true"></i></a>';
				   			...
				   			<a href="javascript:void(0);" onclick="heatsealprint(\''.$order_id.'\');"  data-toggle="modal" class="btn-icon"><i class="fa fa-print" aria-hidden="true"></i></a>
...
$lstatus++;
	...
	function __getorderlinedetails()
	{
		...
		if($customerstatus)
		{
			$newarray=array('status'=>'error','message'=>'Customer id is invalid please contact our administrator.');
			echo json_encode($newarray);exit;
		}
		
		if($barstatus)
		{
			$newarray=array('status'=>'error','message'=>'This barcode was scanned already.');
			echo json_encode($newarray);exit;
		}
		if(intval($barcode) == 0)
		{
		...
		$order_details.=' <div class="row">
                  ...
                  $order_details.='   </div>
                     <div class="col-md-3 pull-right no-padd">
                       ...
                       <button type="button" onclick="updateorderlog();" id="begynne" class="btn-lg green">Bekreft og skriv ut</button>
                      </div>               
                    <div class="clearfix"><input type="hidden" id="ordrenr" name="ordrenr" value="'.$order_id.'" /></div> 
                </div> ';
...
'Your email is empty. Please enter your email address');
	...
	//echo '<pre>';print_r($orderamounts);exit;
		...
		$discount=0;
			...
			$this->payments_model->saldoorderlinePayment($orderid,$payment_type,$in_status);
			$orderpaymentstatus=$this->payments_model->updateorderPaymentstatus($orderid);
		...
		for($i=0;$i<count($this->data['blocks']['ready']);$i++){
			 ...
			 if($result){		  
				...
				$str .='<a href="javascript:void(0);" onclick="ajaxutleveringedit('.$this->data['blocks']['ready'][$i]['id'].')" rel="'.$this->data['blocks']['ready'][$i]['id'].'">
				  <div class="col-md-3" id="orderinfo_'.$this->data['blocks']['iporders'][$i]['id'].'"> #'.$this->data['blocks']['ready'][$i]['id'].' </div>
