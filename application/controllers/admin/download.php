    ...
    function index()
    {
        ...
        ** -----------------------------------------*/
        ...
        switch ($action) {
			...
			case 'calendar':
				 $this->__pdfCalendarReport();
				 break;	
				 
			case 'kassaekredit':
				 $this->__pdfKassekreditReport();
				 break;	
...
//print_r();exit;
		...
		{
			if(intval($sitems['order']) > 0)
			{
				$newarray[$sitems['order']][]=$sitems['amount'];
			}
			
		}
	
	//echo '<pre>';print_r($subarray);exit;
		...
		$this->data['debug'][] = $this->customer_model->debug_data;
            ...
            //set invoice name
             if (!empty($this->data['blocks']['account'])) 
			 {
				...
				$amount=$this->data['blocks']['account'][$i]['amount'];
				if(intval($orderid) > 0)
				{
				...
				{
					$amount=array_sum($newarray[$orderid]);
				}
				
				$str .='<tr>
						<td style="text-align: left;width:12%; padding: 5px 0 5px; font-family: Helvetica, Arial, sans-serif; font-size: 14px; vertical-align: top;">'.$this->data['blocks']['account'][$i]['rdatewy'].'</td>
						<td style="text-align: left; padding: 5px 0 0;width:10%; font-family: Helvetica, Arial, sans-serif; font-size: 14px; font-weight:normal;vertical-align: top; ">'.$this->data['blocks']['account'][$i]['rtime'].'</td>
						<td style="text-align: left; padding: 5px 0 0;width:40%; font-family: Helvetica, Arial, sans-serif;font-size: 14px; font-weight:normal;vertical-align: top; ">'.$this->data['blocks']['account'][$i]['customername'].'</td>
						...
						<td style="text-align: center; padding: 5px 0 0;width:5%; font-family: Helvetica, Arial, sans-serif;font-size: 14px; font-weight:normal;vertical-align: top;vertical-align: top; ">'.strtoupper($in_type).$this->data['blocks']['account'][$i]['in_type'].'</td>
						<td colspan="2" style="text-align: right; padding: 5px 0 0; font-family: Helvetica, Arial, sans-serif;font-size: 14px; font-weight:normal;vertical-align: top;">kr '.formatcurrency($amount).'</td>
					  </tr>';
						if($in_type != 'kk')
						{  
							$total +=	$amount; 
    ...
    function __pdfOrdersReport()
    {
        ...
        if ($next) {
			...
			$str ='';
			$newarray=array();
			foreach($iporders['orderinfo'] as $items)
			{
				$newarray[$items['id']][]=$items['id'];
			}
			   ...
			   $branch =  ($company == $this->data['settings_company']['company_name']) ?  '('.substr($company,0,3) .')':  (($company != $this->session->userdata['partner_branch_name']) ? '('.substr($company,0,3).')' :  ''); 
				...
				//{
					...
					$str .='<tr>
						...
						<td style="text-align: left; padding: 5px 0 0;width:45%; font-family: Helvetica, Arial, sans-serif; font-size: 14px; font-weight:normal;vertical-align: top; ">'.$this->data['blocks']['iporders'][$i]['customername'].'</td>						
						...
						<td style="text-align: left; padding: 5px 0 0;width:20%; font-family: Helvetica, Arial, sans-serif;font-size: 14px; font-weight:normal;vertical-align: top; ">'.$kategori.'</td>
						<td style="text-align: center; padding: 5px 0 0;width:10%; font-family: Helvetica, Arial, sans-serif;font-size: 14px; font-weight:normal;vertical-align: top;vertical-align: top; ">'.$this->data['blocks']['iporders'][$i]['einitial'].' </td>
					<td style="text-align: center; padding: 5px 0 0;width:5%; font-family: Helvetica, Arial, sans-serif;font-size: 14px; font-weight:normal;vertical-align: top;vertical-align: top; ">'.$status.' </td>
				...
				$str.='<tr><td colspan="7" style="text-align: left; padding: 15px 0 15px; font-family: Helvetica, Arial, sans-serif;font-size: 14px; font-weight:normal;vertical-align: top; ">Ingen resultat.</td></tr>';
			}
    ...
    /**
     * generate a pdf for faktura report
     * uses the mpdf class
     * @attribution
     * http://www.mpdf1.com/mpdf/index.php?page=Download
     * http://davidsimpson.me/2013/05/19/using-mpdf-with-codeigniter/     *
     * @param   string $output display on screen or save as file
     */
    function __pdfFakturaListReport()
    {
        //profiling
        $this->data['controller_profiling'][] = __function__;
        //template file
        $this->data['template_file'] = PATHS_COMMON_THEME . 'download/print.faktura.list.html';
        //flow control
        $next = true;
        //load helper
        $this->load->helper('download');
		
		$_GET['from'] = $this->uri->segment(4);
		$_GET['to'] = $this->uri->segment(5);
		$_GET['status'] = $this->uri->segment(6);
		...
		$this->data['vars']['status'] =	$this->input->get('status');	
        ...
        if ($next) {
			...
			$this->data['blocks']['faktura']=$this->settings_faktura_model->getFakturaList($this->input->get('status'));
			$this->data['debug'][] = $this->settings_faktura_model->debug_data;
			
			//print_r($this->data['blocks']['faktura']);
	...
	{
		//$this->process_order_model->createInfiledownload();
		$this->process_order_model->downloadInfile();
		exit;
	}
	

    /**
     * generate a pdf for calendar report
     * uses the mpdf class
     * @attribution
     * http://www.mpdf1.com/mpdf/index.php?page=Download
     * http://davidsimpson.me/2013/05/19/using-mpdf-with-codeigniter/     *
     * @param   string $output display on screen or save as file
     */
    function __pdfCalendarReport()
    {
        //profiling
        $this->data['controller_profiling'][] = __function__;
        //template file
        $this->data['template_file'] = PATHS_COMMON_THEME . 'download/print.calendar.html';
        //flow control
        $next = true;
        //load helper
        $this->load->helper('download');
		...
		$year = $this->data['vars']['year']= $this->uri->segment(5);
		$employee =$this->session->userdata['employee_p_branch'];
		$staff_id=$this->session->userdata['current_staff'];
		
		
		$this->data['vars']['month'] =  $this->general_model->getMonthName($month);
        ...
        if ($next) {
			...
			if(count($this->data['blocks']['timesheet']) > 0){
		  ...
		  $this->data['lists']['faktura'] =  $str;
		  $this->data['lists']['employee_name'] =  $this->session->userdata['current_staff_name'];
		  $this->data['lists']['employee_id'] =   $this->session->userdata['current_staff'];

		$filename = 'Calendar_'.$staff_id.'.pdf';
    ...
    /**
     * generate a pdf for kassae kredit report
     * uses the mpdf class
     * @attribution
     * http://www.mpdf1.com/mpdf/index.php?page=Download
     * http://davidsimpson.me/2013/05/19/using-mpdf-with-codeigniter/     *
     * @param   string $output display on screen or save as file
     */
    function __pdfKassekreditReport()
    {
        //profiling
        $this->data['controller_profiling'][] = __function__;
        //template file
        $this->data['template_file'] = PATHS_COMMON_THEME . 'download/print.kassekredit.html';
        //flow control
        $next = true;
        //load helper
        $this->load->helper('download');
        ...
        if ($next) {
			...
			if(count($this->data['blocks']['complaint']) > 0){
		...
		$filename = 'Kassekredit_'.$month.'_'.$year.'.pdf';
