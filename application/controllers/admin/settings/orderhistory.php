...
class Orderhistory extends MY_Controller
{
    ...
    function index()
    {
        ...
        switch ($action) {
			...
			case 'list':
				//template file
				$this->data['template_file'] = PATHS_ADMIN_THEME . 'settings.deliver.orders.html';
                $this->__sameDayOrderDeliver();
				break;
    ...
    /**
     * orders deliver on the same day placed
     */
    function __sameDayOrderDeliver()
    {
        ...
        if ($next) {
			...
			//orders placed and delivered on the same day
			$this->data['reg_blocks'][] = 'orders';
			$this->data['blocks']['orders'] =$this->settings_order_model->sameDayOrderDeliver();
			
			//print_r($this->data['blocks']['orders']);
				...
				//echo $order_id.'=>'.$ostatus;echo '<br>';
				  ...
				  $ostatus = ($ostatus=='9' || $ostatus=='11') ? 'Ut' : 'Inn';	
					...
					<a href="#" rel="'.$this->data['blocks']['iporders'][$i]['id'].'">
					 ...
					 /*$str .='<div class="col-md-1 text-center"> <div class="'.$color.' paymentstatus"> '.$status.'</div> </div></a><div class="col-md-1 text-center"><div title="utsikt" class="bg-purple paymentstatus utsikt" id="'.$this->data['blocks']['iporders'][$i]['id'].'">PT</div></div><div class="col-md-1 text-center"> <div title="utlever" class="bg-primary  paymentstatus utlever"  id="'.$this->data['blocks']['iporders'][$i]['id'].'">UT</div></div><div style="display:none;" id="kasserer_'.$this->data['blocks']['iporders'][$i]['id'].'">Kasserer: '.$this->data['blocks']['iporders'][$i]['einitial'].'</div>';*/
