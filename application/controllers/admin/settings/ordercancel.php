...
class Ordercancel extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        //template file
        $this->data['template_file'] = PATHS_ADMIN_THEME . 'settings.orders.cancel.html';
    ...
    /**
     * load cancel order listing
     */
    function __cancelorderListing()
    {
		...
		// For order
		$order_cancel=$this->settings_order_model->getCancelOrderhistory();
		$this->data['debug'][] = $this->settings_order_model->debug_data;
		
		//print_r($order_cancel);
		
		// For orderline
		$orderline_cancel=$this->settings_order_model->getCancelOrderhistory(0,'orderline');
		$this->data['debug'][] = $this->settings_order_model->debug_data;
		
		//print_r($orderline_cancel);
		
		$result = array_merge($order_cancel, $orderline_cancel);
		
		//print_r($result);
		
		
		$this->data['blocks']['iporders']= $this->general_model->array_sort_by_column($result, 'cid');
		
		
		//print_r($this->data['blocks']['iporders']);
				...
				//echo $order_id.'=>'.$ostatus;echo '<br>';
				  ...
				  $ostatus = ($ostatus=='9' || $ostatus=='11') ? 'Ut' : 'Inn';	
					...
					<a href="#" rel="'.$this->data['blocks']['iporders'][$i]['id'].'">
					  ...
					  $str .='<div class="col-md-3"> #'.$this->data['blocks']['iporders'][$i]['id'].' <span class="green-text">'.$branch.'</span></div>
...
//<div class="col-md-1 text-center"><div title="utsikt" class="bg-purple paymentstatus utsikt" id="'.$this->data['blocks']['iporders'][$i]['id'].'">UT</div></div>					
	...
	function __approvecancel()
	{
		...
		if(count($_POST) > 0)
		{
			...
			$res=$this->settings_order_model->approvecancel($orderid,$status);
			if($res)
			{
				$result = array("status"=>'success',"message"=>"Cancel has been approved successfully!");
			}
			else
			{
				$result = array("status"=>'error',"message"=>"Already approved");
			}
