...
class Printecspos extends MY_Controller
{
	...
	function __printSaldo($printtype)
	{
		...
		$paidsaldo = $data['paid'];
			...
			$employee =  $this->employee_model->getEmployeeDetail($customer['employee']);
			$cmds .= 'Kasserer: '.$employee['initial'];
			$cmds .= $newLine;
	...
	function __printTag($printtype)
	{
					  ...
					  else
					  {
						...
						if($qty > 0)
						{
							foreach(range(1,$qty) as $qtys)
							{
								...
								$cmds .= $esc . '!' . '0x38'; //Emphasized + Double-height + Double-width mode selected (ESC ! (8 + 16 + 32)) 56 dec => 38 hex
								$cmds .= $orderid;
								$cmds .= str_pad($orderid.' '.$companyname.' '.$partnerinfo['id'], 0, "", STR_PAD_LEFT).'       ';
								$cmds .=$customer['customer_name'];
								$cmds .= $newLine;
			
								$cmds .= $partnerinfo['id'].':'.$ready;
								$cmds .= str_pad('I '.date('d/m',strtotime($orderinfo['order_time'])), 0, "", STR_PAD_LEFT).'       ';
								$cmds .='Rdy '.$ready;
								$cmds .= $newLine;
								
								$cmds .=$weekday.':'.$item.'/'.$total_qty;
								$cmds .=str_pad($olineitmes['name'], 0, "", STR_PAD_LEFT).'       ';
								$cmds .=$customer['number'];
								$cmds .= $newLine;
								$cmds.=$pagebreak;
								$item ++;
		...
		$cmds='0x0A0x0A0x0A0x0A0x0A0x1D0x560x000x0A0x1B!0x380x1Ba0x01Vinderen0x0A0x1B!0x00Slemdalsveien 720x0A0373 OSLO0x0ATelefon: 0x2B47 224919190x0AOrg.nr: 938 165 238 MVA0x1Ba0x000x0A0x0A0x1Ba0x010x1Dh0x3C0x1D0x6B0x05101552130x000x0A0x1B!0x38101552130x000x0A0x0A0x1B!0x000x1Ba0x00979700350x0A0x1B!0x38Customer, Shop0x1B!0x000x0AKasserer: PK0x0A24.03.2017 12:29:250x0A0x0A1    Skjorte                    0x1Ba0x02kr 26,000x0A0x1Ba0x000x0A0x1B!0x20Ferdig: 0x0ASkjorte0x0Aons 29/03/20170x0A0x0A0x1Ba0x01 Antall 10x0A0x0A0x1B!0x240x1B!0x24-Intern kopi-0x0A0x1Ba0x000x0A0x0A0x0A0x0A0x0A';
		
		//profiling
        $this->data['controller_profiling'][] = __function__;
		echo '<input type="hidden" value="true" name="pid" id="pid" /> ';
		echo '<input type="hidden" name="printerCommands" id="printerCommands" value="'.$cmds.'" /> ';
       ...
       // $cmds .= $this->data['fields']['customer']['address'];
		...
		//print_r($orderdetails);
			...
			for($i=0;$i< count($orderdetails);$i++)
			{
				...
				$orderdetails[$i]['name'] = htmlentities($arr['name'], ENT_QUOTES, "UTF-8");
				
				
				//$orderdetails[$i]['name']='D8 &#216; &Oslash;';
				
				
				//$orderdetails[$i]['name'] = html_entity_decode($orderdetails[$i]['name'], ENT_QUOTES, "UTF-8");
				...
				foreach($categoryorderline as $catkey=>$catitems)
				{
					...
					if(count($catitems) > 0)
					{
						...
						foreach($catitems as $key=>$cat_html)
						{
							...
							$catdelivery .= $catitems[$key]['delivery'].$newLine;
						...
						$categoryprint .= $newLine;
						$categoryprint .= $newLine;
						$categoryprint .= $esc . '!' . '0x24'; //Emphasized + Double-height + Double-width mode selected (ESC ! (8 + 16 + 32)) 56 dec => 38 hex
		...
		//}
				...
				//echo '<pre>';print_r($categoryprint);exit;
	...
	function __testprintBill($printtype)
    {
		...
		$cmds='0x1B@0x1B!0x381015521310155213 Vi 1       Customer, Shop0x0A1:31/03I 24/03       Rdy 31/030x0Afre:1/1Jakke kort m/ silkefor       979700350x0A0x0A0x0A0x0A0x0A0x0A0x1D0x560x000x0A';
