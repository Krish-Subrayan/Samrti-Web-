...
class Orders extends MY_Controller
{
	...
	//echo '<pre>';print_r($this->data['blocks']['iporders']);exit;
     ...
     function __getOrderDetail(){
		...
		//get results
		$this->data['reg_blocks'][] = 'orderline';
		$this->data['blocks']['orderline'] = $orderline = $this->orders_model->getOrderLine($order_id);
		$this->data['debug'][] = $this->orders_model->debug_data;
		...
		//print_r($this->data['blocks']['orderline']);
				...
				for($i=0;$i<count($this->data['blocks']['orderline']);$i++){ 
					...
					if (round($quantity, 0) == $quantity)
					{
					 ...
					 $productPrice=round($productPrice);
					  ...
					  if($this->data['blocks']['orderline'][$i]['payment_status'] == 'canceled')
					  {
							$deliverystatus=1;
					  }
					...
					$this->data['blocks'][$cart_type][$k]['payment_method']='';
					
					/*if($this->data['blocks']['orderline'][$i]['payment_status'] == 'paid' || $this->data['blocks']['orderline'][$i]['payment_status'] == 'waiting')
					{*/
			
						$this->data['blocks'][$cart_type][$k]['payment_method'] = $this->general_model->getInType($this->data['blocks']['orderline'][$i]['payment_type']);
						...
						if($orderlineemp)
						{
							$this->data['blocks'][$cart_type][$k]['orderline_emp']='('.$orderlineemp['initial']. ' - ' . $this->data['blocks'][$cart_type][$k]['payment_method'].')';
						}
			...
			//echo '<pre>';print_r($vask_arr);exit;
						...
						//	$str_vask.='<div class="round"><div class="img">'.$vask_arr[$z]['qty'].'</div></div>';
						 ...
						 <div class="pull-left col-md-5 no-padd">
						  ...
						  if($vask_arr[$z]['status'] == 'pending')
						  {
							  ...
							  $status = ($vask_arr[$z]['status']=='canceled') ? 'C' : (($vask_arr[$z]['status'] == 'paid') ?  'BE' : 'UB');	
					...
					//	$str_rens.='<div class="round"><div class="img">'.$rens_arr[$z]['qty'].'</div></div>';
						...
						$str_rens.='   </div>
						  ...
						  $resstatus=$this->settings_order_model->getCancelstatus($order_id,$rens_arr[$z]['id']);
							  ...
							  $status = ($rens_arr[$z]['status']=='canceled') ? 'C' : (($rens_arr[$z]['status'] == 'paid') ?  'BE' : 'UB');	
			...
			*/
			...
			$odate = date('d.m.Y',strtotime($orderinfo['order_time']));
			$func = (strtotime($odate) != strtotime(date('d.m.Y'))) ? 'editOrdermodal()' : 'confirmBetal()';
		...
		$this->data['visible']['bekreft_status']=$this->process_order_model->getOrderlogstatus($order_id,5);
			...
			else
			{
				...
				if($binfo)
				{
					...
					{
						$result = $this->orders_model->getOrderinfo($barcode);
						if(!empty($result)){
						  $this->data['blocks']['blk1'][0] = $result;
						}
					}
...
$lstatus++;
	...
	function __updateorderlog()
	{
		...
		{	
			...
			if($status)
			{
				...
				if($saldo_status){
					$status = $this->makeSkjortePayment($orderid);
				}
	...
	/*Payment for skjorte when once scanned for saldo customer  */
	function makeSkjortePayment($orderid)
	{
		//$request = $_POST;
		$saldostatus=true;
...
<p><b>Totalt</b></p>
 ...
 <hr><div class="popupscroll">
			...
			//	echo $data[$j]['id'];exit;
					...
					$data[$j]['payment_status'] = ($data[$j]['payment_status']=='pending' && $data[$j]['payment_type'] =='invoice') ? 'paid' : $data[$j]['payment_status'];
					...
					<div class="pull-left col-md-12 no-padd text-right">
				 	 <div class="saldo-button1 group-pos">
								...
								<button type="button" id="pay_cash" onclick="paymenttype(\'cash\')" data-switch-set="#discount_type12" data-switch-value="Kontant" class="btn btn-default payment_type">Kontant</button>								
                                  <button type="button" id="pay_invoice" onclick="paymenttype(\'invoice\')" data-switch-set="#discount_type12" data-switch-value="Faktura" class="btn btn-default payment_type">Faktura</button>
                                  <button type="button"  id="pay_visa"   onclick="paymenttype(\'visa\')"  data-switch-set="#discount_type12" data-switch-value="Kort" class="btn btn-default payment_type">Kort</button>
                                  <button type="button"  id="pay_saldo"   onclick="paymenttype(\'saldo\')"  data-switch-set="#discount_type12" data-switch-value="Saldo" class="btn btn-default payment_type">Saldo</button>
...
';
	...
	function __makeorderpayment()
	{
		...
		$orderid=$_POST['eorder_id'];
		$in_type=$_POST['opay_type'];
		//$saldostatus = ($in_type == 'invoice')  ? (($_POST['saldostatus']=='1') ? '1' : '0' ) : (($in_type == 'saldo') ?  '1' : '0' );
		
		$saldostatus = ($in_type == 'saldo') ?  '1' : '0' ;
		
		$orderdetails = $this->orders_model->getOrderLine($orderid);
		$pending_count = $this->orders_model->getPendingOrderlines($orderid);
...
'Your email is empty. Please enter your email address');
		...
		{	
			...
			if($totalorderline == count($_POST['corderlines']))
			{
				...
				$this->orders_model->orderlinePayment($orderid,0,$payment_type,$in_status,$checkedoderline,$pending_count);
			...
			else
			{
				...
				if($saldostatus)
				{
						...
						$this->orders_model->orderlinePayment($orderid,$orderline,$payment_type,$in_status,$checkedoderline,$pending_count);
						$orderpaymentstatus=$this->payments_model->updateorderPaymentstatus($orderid);
				}
	...
	$str.='<input type="hidden" name="checkedoderline" id="checkedoderline" value="" />';
				...
				//	$total = ($delsum + $delivery_charge) - $discount ;
					...
					if($data[$j]['payment_status'] == 'pending')
					{
						...
						if($deliverystatus)
						{
							...
							if(strtolower($data[$j]['payment_status']) == 'pending' && strtolower($data[$j]['payment_type']) != 'invoice')
							{
								$waitamountarray[]=$productPrice;
								$paidstatus=0;
								$discountstatus=0;
	   ...
	   <input type="hidden" value="'.$productPrice.'" name="ordertotal['.$data[$j]['id'].']" id="oltotal_'.$data[$j]['id'].'" />
		...
		$saldostatus=$this->payments_model->getSaldostatus($customer);	
			...
			<input type="hidden" id="saldo_amt" name="saldo_amt" value="'.$saldo.'" />
			<input type="hidden" value="'.$discountstatus.'" id="discountstatus" name="discountstatus" />
			   <div class="row col-md-11 mt-sm paid-summary">';
...
$i++;				  
                    ...
                    <div class="col-md-12 no-padd">
                      <div class="row mt-sm">
                        ...
                        <div class="pull-left col-md-2 no-padd mt-sm">
                          <button type="submit" id="add_productbtn"  style="font-size:16px !important;width:70%;padding:8px 0" class="btn-lg btn-primary">Add</button>
	 ...
	 function __getpayorderdetails()
	 {
	  ...
	  {	
	  	if($orderamt > 0 ){
				$htmlout.='<div class="row">
						<div class="pull-left col-md-12 no-padd text-right">
							<div class="saldo-button1 group-pos">
								<div class="btn-group pull-right">
									<button type="button" id="mpay_cash" onclick="multipaymenttype(\'cash\')" data-switch-set="#discount_type12" data-switch-value="Kontant" class="btn btn-default payment_type">Kontant</button>
									<button type="button" id="mpay_invoice" onclick="multipaymenttype(\'invoice\')" data-switch-set="#discount_type12" data-switch-value="Faktura" class="btn btn-default payment_type">Faktura</button>
									<button type="button"  id="mpay_visa"   onclick="multipaymenttype(\'visa\')"  data-switch-set="#discount_type12" data-switch-value="Kort" class="btn btn-default payment_type">Kort</button>
									
								</div>
								<input type="hidden" name="ompay_type" id="ompay_type">
							</div>
							<div class="clearfix"></div>
						</div>
					</div>';
	  	...
	  	if($orderamt > 0 ){
				$htmlout.='<div class="row">
						<div class="pull-left col-md-12 no-padd text-right">
							<div class="saldo-button1 group-pos small-buttons">
								<div class="btn-group pull-right">
									<button type="button" id="mpay_cash" onclick="multipaymenttype(\'cash\')" data-switch-set="#discount_type12" data-switch-value="Kontant" class="btn btn-default payment_type">Kontant</button>
									<button type="button" id="mpay_invoice" onclick="multipaymenttype(\'invoice\')" data-switch-set="#discount_type12" data-switch-value="Faktura" class="btn btn-default payment_type">Faktura</button>
									<button type="button"  id="mpay_visa"   onclick="multipaymenttype(\'visa\')"  data-switch-set="#discount_type12" data-switch-value="Kort" class="btn btn-default payment_type">Kort</button>
									<button type="button"  id="mpay_saldo"   onclick="multipaymenttype(\'saldo\')"  data-switch-set="#discount_type12" data-switch-value="Saldo" class="btn btn-default payment_type">Saldo</button>
								</div>
								<input type="hidden" name="ompay_type" id="ompay_type">
							</div>
							<div class="clearfix"></div>
	 ...
	 function __paymentMultiOrder($self=false,$orderid=0)
	 {
		...
		{	
			foreach($_POST['orders'] as $order_id)
			{
				...
				$data = $this->orders_model->getOrderLine($order_id,'pending');
				...
				{
					$price =  '';
					if($data[$j]['payment_status'] == 'pending' && $data[$j]['payment_type'] != 'invoice'){
				...
				{	
					if($totalorderline == count($corderlines))
					{
						...
						//$saldostatus = ($in_type == 'invoice')  ? (($saldostatus=='1') ? '1' : '0' ) : (($in_type == 'saldo') ?  '1' : '0' );
						$saldostatus = ($in_type == 'saldo') ?  '1' : '0' ;
						...
						$this->orders_model->orderlinePayment($orderid,0,$payment_type,$in_status,count($corderlines),$totalorderline);
						$orderpaymentstatus=$this->payments_model->updateorderPaymentstatus($orderid);
					...
					else
					{
						...
						foreach($corderlines as $orderline)
						{
								...
								$this->orders_model->orderlinePayment($orderid,$orderline,$payment_type,$in_status,count($corderlines),$totalorderline);
								$orderpaymentstatus=$this->payments_model->updateorderPaymentstatus($orderid);
						}
	 ...
	 function __updatemultideliverystatus()
	{
		...
		function __logReceiptWithHeatSeal()
		{
			...
			$this->data['controller_profiling'][] = __function__;
			//$order_id = 20154586;
			$order_id = $barcode = $this->uri->segment(4);
			...
			if($baginfo)
			{
				...
				$order_id=$this->data['blocks']['blk1'][0]['id'];	
			}
			else
			{
