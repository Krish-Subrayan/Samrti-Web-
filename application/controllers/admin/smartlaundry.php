...
class Smartlaundry extends MY_Controller
{
    /**
     * constructor method
     */
    public function __construct()
    {
        ...
        //template file
        $this->data['template_file'] = PATHS_ADMIN_THEME . 'smart.laundry.products.html';
        //css settings
    ...
    function index()
    {
		...
		if($this->session->userdata['partner_branch'] != 1000){ //only laundry login can access this page
		...
		if($this->session->userdata['customer']['zip'] == '')
		{
			$this->session->set_flashdata('notice-error', 'Please update zip code');
			redirect('/admin/customer/profile');
			exit();
		}
		else
		{
				$zip=$this->session->userdata['customer']['zip'];
				if(trim($zip) != '')
				{
					$result=$this->general_model->isValidZip($zip);
					if($result)
					{
					...
					{
						$this->session->set_flashdata('notice-error', $this->data['lang']['lang_invalid_zip']);
						redirect('/admin/customer/profile');
						exit();
					}
				}
				 ...
				 $vaskstatus=1;
			  	for($z=0;$z<count($vask_arr);$z++){ 
					...
					if(($z==0) || ($date!=$prev_date)){
						...
						$str_vask.='<div class="row" style="display:none">
			 ...
			 if(count($rens_arr) >0 ){
			  ...
			  for($z=0;$z<count($rens_arr);$z++){
					...
					if(($z==0) || ($date!=$prev_date)){
						...
						$str_rens.='<div class="row" style="display:none">
						<div class="pull-left col-md-3 no-padd text-center">
						</div> 
						<div class="pull-right col-md-6 no-padd  customer-info">
						   <div class="input-group" >
	...
	/*get  time slots in a date  */
	function __getTimeslots(){
		
        //profiling
        $this->data['controller_profiling'][] = __function__;
		$type = $this->input->post('hltype');
		$date = date('Y-m-d',strtotime($this->input->post('utlevering')));
		if($type == 'henting')
		{
			$ttype='collection';
			//$date='2017-03-31';
        ...
        $data = $this->smart_laundry_model->getTimeSlots($ttype,$date,$zone);
        $data = $this->smart_laundry_model->getTimeSlotsHTML($data);
		$result = array("time"=>$data);
		echo json_encode($result);exit;
			...
			for($i=0;$i<count($this->data['blocks']['category']);$i++){
			 ...
			 // echo $count."===".$this->data['blocks']['category'][$i]['id']."=".$this->data['blocks']['category'][$i]['name']."<br>";
			  ...
			  if( $count != ''){
				...
				//$cname = $this->general_model->trim_text($this->data['blocks']['category'][$i]['name'],10);

				$cname = $this->general_model->trim_text($this->data['blocks']['category'][$i]['name']);				
				 
				  $str .='<div class="col-md-2 no-padd text-center  '.$first.'"> 
					<div class="pblock" style="background:#'.$color.'"><a class="catclass" rel="0"  id="'.$this->data['blocks']['category'][$i]['id'].'" href="#">'.$cname.'</a></div>
				  </div>';
...
$(\'a.subcat\').click(function(e){
    ...
    {	
		...
		{
			$orderid = $this->smart_laundry_model->saveOrder($request,$payment_status);
			if($orderid)
			{
