...
class Login extends MY_Controller
{
    ...
    function flmLogOut()
    {
		...
		/*$results = $this->employee_model->stopAllLogin();
    ...
    function __flmPasswordReminder()
    {
        ...
        if ($this->teamprofile_model->checkRecordExists('team_profile_email', $team_profile_email)) {
            ...
            if ($this->teamprofile_model->resetPasswordSetup($team_profile_email, $random_code)) {
                ...
                /** --------------------------emailer - v2----------------------------------------------------------*/
                //vars
                $vars['to_name'] = $team_profile_full_name;
                $vars['url_reset_link'] = $password_reset_link;
                $vars['to_email'] = $team_profile_email;

                //email - client
                $this->__emailer('password_reset_admin', $vars);
                /** --------------------------emailer - v2----------------------------------------------------------*/
    ...
    function __flmLoginSession($results = '')
    {
        ...
        if (is_array($results)) {
			...
			$type = ($results['id'] == 1000) ?  's_laundry' :  'shop';  //when logged in as smart laundry
            ...
            $session_data = array(
                ...
                'partner_email' => $results['partner_email'],
                'partner_vat_nr' => $results['partner_vat_nr'],
                'partner_branch' => $results['id'],
                'partner_branch_name' => $results['name'],
                'partner_branch_phone' => $results['phone'],
                'partner_branch_zip' => $results['zip'],
                'partner_branch_city' => $results['city'],
				'bil_printer' => $results['bil_printer'],
				'tag_printer' => $results['tag_printer'],
                'partner_branch_street' => $results['street']); //set the session - update coookie
            
			$this->session->set_userdata($session_data);
			$newdata = array('session_date'=>date('Y-m-d'));
