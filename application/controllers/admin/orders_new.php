     ...
     function __getOrders(){
		...
		for($i=0;$i<count($this->data['blocks']['ready']);$i++){
			 ...
			 if($result){		  
				...
				<a href="#" rel="'.$this->data['blocks']['ready'][$i]['id'].'">
			...
			if($result)
			{
				...
				$artikler = $this->general_model->trim_text($artikler);
				  ...
				  $status = ($opstatus=='canceled') ? 'C' : (($opstatus == 'paid') ?  'BE' : 'UB');	
					...
					$str .='<a href="#" rel="'.$this->data['blocks']['iporders'][$i]['id'].'">
					
					  <div class="col-md-3" id="orderinfo_'.$this->data['blocks']['iporders'][$i]['id'].'"> #'.$this->data['blocks']['iporders'][$i]['id'].' <span class="green-text">'.$branch.'</span></div>
					 ...
					 $str .='  <div class="col-md-3">kr '.formatcurrency($amount).' </div>';
					 /*$str .='<div class="col-md-1 text-center"> <div class="'.$color.' paymentstatus"> '.$status.'</div> </div></a>';*/
					 ...
					 <div style="display:none;" id="kasserer_'.$this->data['blocks']['iporders'][$i]['id'].'">Kasserer: '.$this->data['fields']['employee']['initial'].'</div>';
					$str .='</div>';				
			}	
		  }//for
		   ...
		   $branch =  ($company == $this->data['settings_company']['company_name']) ?  '('.substr($company,0,3) .')':  (($company != $this->session->userdata['partner_branch_name']) ? '('.substr($company,0,3).')' :  ''); 
			...
			$artikler = $this->general_model->trim_text($artikler);
			  ...
			  $status = ($opstatus=='canceled') ? 'C' : (($opstatus == 'paid') ?  'BE' : 'UB');	
				...
				<a href="#" rel="'.$this->data['blocks']['delivered'][$i]['id'].'">
				  <div class="col-md-2" id="orderinfo_'.$this->data['blocks']['delivered'][$i]['id'].'"> #'.$this->data['blocks']['delivered'][$i]['id']. ' <span class="green-text">'.$branch.'</span></div>
				  ...
				  <div class="col-md-2 ">kr '.formatcurrency($amount).' </div>
				  <div class="col-md-1 text-center "> <div class="'.$color.' paymentstatus"> '.$status.'</div></div></a><div class="col-md-1 text-center"><div title="utsikt" class="bg-purple paymentstatus utsikt" id="'.$this->data['blocks']['delivered'][$i]['id'].'">UT</div></div><div style="display:none;" id="kasserer_'.$this->data['blocks']['delivered'][$i]['id'].'">Kasserer: '.$this->data['fields']['employee']['initial'].'</div>
				</div>';				
			...
			$ordered_branch =  ($company != $this->session->userdata['partner_branch_name']) ? $company : '';
             ...
             $company_info ='<div class="company-info">
				...
				$company_info .='<a href="#editproduct" onclick="editOrder(\''.$order_id.'\');" id="editproductbtn" data-toggle="modal" class="btn-icon"><i class="fa fa-edit" aria-hidden="true"></i></a>
				   			...
				   			<a href="#orderHeatSeal" onclick="damageheatseal(\''.$order_id.'\');"  data-toggle="modal" class="btn-icon"><i class="fa fa-camera" aria-hidden="true"></i></a>
							
				   			<a href="javascript:void(0);" onclick="heatsealprint(\''.$this->data['vars']['site_url']."admin/orders/print-heatseal/".$order_id.'\');"  data-toggle="modal" class="btn-icon"><i class="fa fa-print" aria-hidden="true"></i></a>
		...
		$this->data['blocks']['blk1'][$i]['dtime']  = date("H:i",strtotime($this->data['blocks']['blk1'][$i]['dstime'])) ."->" . date("H:i",strtotime($this->data['blocks']['blk1'][$i]['detime']));
			...
			if(!empty($data)){
				...
				for($j=0;$j<count($data);$j++){
					...
					<div class="pull-left col-md-3 no-padd" '.$onclick.'>
                   <p>'.$data[$j]['name'].'</p>
                   </div>  
...
$lstatus++;
	...
	function __getorderlinedetails()
	{
		...
		if($barstatus)
		{
			$newarray=array('status'=>'error','message'=>'This orderline barcode already scanned');
			echo json_encode($newarray);exit;
		}
		if(intval($barcode) == 0)
		{
		...
		{
			$newarray=array('status'=>'error','message'=>'Invalid orderline');
			echo json_encode($newarray);exit;
		}
		$heat_seal=$this->process_order_model->heatsealinfo($barcode,$orderline,$producttype);
		$this->process_order_model->updateheatseal_log($heat_seal,$orderline,'started');
...
<div class="col-md-1"><input type="checkbox" onclick="paycheckall(this)" id="checkall">
</div>';
 ...
 <hr><div class="popupscroll">
			...
			//	echo $data[$j]['id'];exit;
					...
					if(count($data) == $lstatus)
					{
						if(intval($newquantity) > 0)
						{
							$str.=' <div id="tr_'.$data[$j]['id'].'" class="orderlist '.$class.'">';
						}
						else
						{
							$str.=' <div id="tr_'.$data[$j]['id'].'" class="orderlist '.$class.'">';
				...
				if($data[$j]['p_b_delivery_time'] != '' && $data[$j]['p_b_delivery_time'] != '0000-00-00')
				{
					...
					$disable = ($data[$j]['product']=='1') ? "disabled" : '';
					
					
					$str .= '
					<div class="pull-left col-md-1">
					
					<input onclick="payitem(this);" type="checkbox" '.$disable.' name="printorderlines[]" id="print_'.$data[$j]['id'].'" value="'.$data[$j]['id'].'" />
					</div>
					';
                  ...
                  $str .= ' <div class="pull-left col-md-2 no-padd text-center">
					...
					<div class="pull-left col-md-2 no-padd text-center">
						 ...
						 <a class="editprod" id="edit_pad_'.$data[$j]['id'].'" data-toggle="modal" data-title="'.$data[$j]['name'].'"  href="#editProductModal" data-stuff="'.$data[$j]['id'].'@'.$data[$j]['name'].'@'.$data[$j]['price'].'@'.$data[$j]['path'].'@'.$data[$j]['description'].'@'.$data[$j]['quantity'].'@'.$data[$j]['ptype'].'@'.$data[$j]['complain'].'@'.$data[$j]['in_house'].'@'.round($data[$j]['oprice']).'" style="vertical-align: middle; line-height:inherit">'.$data[$j]['quantity']. '</a>';	
...
<div class="col-md-1"><input type="checkbox" onClick="paycheckall(this)" id="checkall"  /></div>';



if(($order_from == 'shop')){

$str .='<div class="pull-left col-md-2">
<p><b>Artikler</b></p>
</div>
 ...
 <hr><div class="popupscroll">
			...
			//	echo $data[$j]['id'];exit;
					...
					if(count($data) == $lstatus)
					{
						if(intval($newquantity) > 0)
						{
							$str.=' <div id="trr_'.$data[$j]['id'].'" class="orderlist order'.$data[$j]['payment_status'].'">';
						}
						else
						{
							$str.=' <div id="trr_'.$data[$j]['id'].'" class="orderlist order'.$data[$j]['payment_status'].'">';
				...
				if($data[$j]['p_b_delivery_time'] != '' && $data[$j]['p_b_delivery_time'] != '0000-00-00')
				{
					...
					<p><a class="removeOrderline" onclick="updateorderlines(\''.$data[$j]['id'].'\',\''.$order_id.'\');"  href="#removeOrderlineModal" data-toggle="modal" data-title="Remove" data-stuff="'.$data[$j]['id'].'@'.$order_id.'"><span class="x-icon"><i class="icon-remove red-text"></i></span></span></a>
...
$summary.='</select>';
                  ...
                  <button type="button" id="add_productbtn" onclick="addProduct();" style="font-size:14px !important;width:70%;padding:8px 0" class="btn-lg btn-primary">'.$this->data['lang']['lang_add_product'].'</button>
...
';
		...
		$order_details.='<div class="row mt-sm" style="margin-bottom:30px">
					...
					else
					{
                     ...
                     <div class="col-md-3 pull-right">
                       <button type="button" onclick=""  class="btn-lg orange"  id="order_close"  data-dismiss="modal" aria-hidden="true">'.$this->data['lang']['lang_update'].'</button>
					</div>
	...
	function __getOrderReceipt()
	{
		...
		$deliveryinfo=$this->orders_model->getCollectionDeliverytime('delivery',$orderinfo['delivery_time']);
		   ...
		   $customer_detail='<div class="customer-detail mt-sm">
			...
			//	echo $data[$j]['id'];exit;
					...
					$str .= '</div> <div class="pull-left col-md-3 no-padd" '.$onclick.'>
                   <p>'.$data[$j]['name'].'</p>
                   </div>
					<div class="pull-left col-md-2 no-padd text-center" '.$onclick.'>
                   <p>'.$p_b_delivery_time.'</p>
		...
		$order_details.=' <div class="row">
                  ...
                  $order_details.='   </div>
                     <div class="col-md-3 pull-right no-padd">
                        <!--[onshow;block=div;when [visible.wi_show_process_button] == 1;comm]-->
                       <button type="button" onclick="updateorderlog();" id="begynne" class="btn-lg green">Bekreft</button>
                      </div>               
                    <div class="clearfix"><input type="hidden" id="ordrenr" name="ordrenr" value="'.$order_id.'" /></div> 
                </div> ';
		$result = array("order_details"=>$order_details);
...
'Your email is empty. Please enter your email address');
		...
		{	
			...
			$newarray=array('status'=>'success','orderid'=>$orderid,'message'=>'Payment status has been updated','orderpaymentstatus'=>$orderpaymentstatus,'amount'=>$amount);
			echo json_encode($newarray);exit;
    ...
    {	
		...
		if($this->session->userdata['cart_contents'])
		{
			...
			if($orderid)
			{
				...
				$result = array("status"=>'success',"message"=>"Bestillingen Har blitt plassert. Ordrenummer er #$orderid",'orderid'=>$orderid,'customer'=>$this->session->userdata['customer']['id']);
				$this->cart->destroy();
			}
			else
			{
		...
		function __printReceipt(){
			...
			if($order_from == 'shop'){
            	...
            	$employee = '<li style="margin:0; padding: 5px 0 0; font-family: \'arial\', monospace; font-size: 12px; font-weight: normal; color: #000; list-style: none;">Kasserer: [employee.initial]</li>';
			...
			for($i=0;$i< count($orderdetails);$i++)
			{
				...
				$html .='<tr>
                <td nowrap="nowrap" style="text-align: left;width:18%; padding: 5px 0 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:bold;  vertical-align: top;">'.$quantity.'</td>
                <td style="text-align: left; padding: 5px 0 0;width:70%; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;vertical-align: top; ">'.$orderdetails[$i]['name'];
				...
				if($orderdetails[$i]['special_instruction']!=''){
					$html .='<p style="margin-top: 0px; margin-bottom: 0px; padding: 0px;font-size: 13px; line-height:28px"><b>Fritekst :</b><br>'.$orderdetails[$i]['special_instruction'].'</p>';
				}
				
				if($orderdetails[$i]['complain']==1){
					$html .='<p style="margin-top: 0px; margin-bottom: 0px; padding: 0px;font-size: 13px; line-height:28px"><b>Reklamasjon</b></p>';
				}
				
				if($orderdetails[$i]['in_house']==1){
					$html .='<p style="margin-top: 0px; margin-bottom: 0px; padding: 0px;font-size: 13px; line-height:28px"><b>Renses på huset</b></p>';
				}
				...
				$html .='</td>
                <td nowrap="nowrap" style="text-align: right;width:20%; padding: 5px 0 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;vertical-align: top;">kr '.formatcurrency($productPrice).$vary.'</td>
              </tr>';
			  $str.=$html;
			...
			if($orderinfo['type'] != 'shop')
			{
				...
				$summery='<tr>
				 <td colspan="2" style="text-align: right; padding:5px; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Delsum'.$min_price_txt.'</td><td nowrap="nowrap" style="text-align: right; padding:5px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal; border-top:#000 dashed 1px;">kr '.formatcurrency($subtotal).'</td>
				  </tr>';
		...
		if($discount > 0){
			...
			$summery.='<tr>
             <td colspan="2" style="text-align: right; padding:5px; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Discount ('.$vouchercode.')</td><td nowrap="nowrap" style="text-align: right; padding:5px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal; border-top:#000 dashed 0px;">kr '.formatcurrency($discount).'</td>
              </tr>';
			...
			if($orderinfo['type'] != 'shop')
			{
				if($fprice < $min_price)
				{
					...
					$summery.='<tr>
					 <td colspan="2" style="text-align: right; padding:5px; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Minste beløp</td><td  nowrap="nowrap" style="text-align: right; padding:5px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal; border-top:#000 dashed 0px;">kr '.formatcurrency($min_price).'</td>
					  </tr>';
		...
		$frakt=$delviery;
			...
			{
				$summery.='<tr>
				 <td colspan="2" style="text-align: right; padding:5px; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Levering</td><td   nowrap="nowrap" style="text-align: right; padding:5px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal; border-top:#000 dashed 0px;">kr '.formatcurrency($delviery).'</td>
				</tr>';
			}  
			  //mva
			  ...
			  $summery.='<tr>
                <td colspan="2" style="text-align: right; padding:0px; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Herav 25% MVA</td>
                <td style="text-align: right; padding:5px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;">kr '.formatcurrency($mva).'</td>
              </tr>
              <tr>
                <td colspan="2" style="text-align: right; padding:5px; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Totalt</td>
                <td style="text-align: right; padding:5px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal; border-top:#000 dashed 1px; border-bottom:#000 dashed 1px; ">kr '.formatcurrency($this->data['fields']['order']['totalt']).'</td>
              </tr>';
			  
			  if($saldo_amount != '' && $saldo_amount != '0,00')
			  {
				  $summery.=' <tr><td colspan="3" style="text-align: left; padding: 10px 0 0;; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Kassakredit saldo: kr '.$saldo_amount.'</td></tr>';
			  }
			
			   
			   if($meter_text == 1){
			   $summery.=' <tr>
              <td colspan="3" valign="top" align="left" style="text-align: left; padding: 10px 0 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;"><i>* Prisen blir kalkulert når ferdig</i></td>
              </tr>';
			   }
			...
			if($this->data['fields']['order']['delivery_note'] != '')
			{
				 $this->data['lists']['delivery_note'] = '<tr>
				  <td valign="top" colspan="3" align="left" style="margin:0; padding:0; font-family: \'arial\', monospace; font-size: 13px; font-weight: bold;  color: #000">Notater</td>
				</tr>
				<tr>
				  <td colspan="3" valign="top" align="left" style="text-align: left; padding: 10; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;">'.$this->data['fields']['order']['delivery_note'].'</td>
				</tr>';
			}
			...
			if($this->data['fields']['order']['special_instruction'] != '')
			{
				 $this->data['lists']['special_instruction'] = '<tr>
				  <td valign="top" colspan="3" align="left" style="margin:0; padding:0; font-family: \'arial\', monospace; font-size: 13px; font-weight: bold;  color: #000">Spesialinstruksjoner</td>
				</tr>
				<tr>
				  <td colspan="3" valign="top" align="left" style="text-align: left; padding: 10; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;">'.$this->data['fields']['order']['special_instruction'].'</td>
				</tr>';
			}
			...
			if(count($checkdeliverydate) == 1)
			{
				...
				$this->data['lists']['orderlinedelivery'] = '<tr>
				  <td colspan="3" style="margin:0px 0; padding:10px 0; font-family: \'arial\', monospace; font-size: 20px; font-weight: bold;  color: #000" valign="top" align="left">Ferdig: '.$delivery_dates.'</td>
				</tr>';
			}
			else
			{
				if(count($orderlinedelivery) > 0)
				{
					...
					$this->data['lists']['orderlinedelivery'] = '<tr>
				  <td colspan="3" style="margin:0px 0; padding:10px 0; font-family: \'arial\', monospace; font-size: 20px; font-weight: bold;  color: #000" valign="top" align="left">Ferdig: </td>
				</tr><tr>
					<td valign="top" colspan="3" align="left" style="margin:0px 0; padding:10px 0; font-family: \'arial\', monospace; font-size: 20px; font-weight: bold;  color: #000">'.$orderlinedelivery_time.'</td>
					</tr>';
				}
			}
			...
			$commonheader=' <table  width="90%" align="center" border="0" cellpadding="0" cellspacing="0" > 
    <tbody>
      <tr>
        <th valign="top" align="center" style="padding:0 0px 13px 0;"><ul style="margin:0; padding: 0; font-family: \'arial\', monospace; font-size: 15px; font-weight: normal;text-align:center;">
            ...
            <li style="margin:0; padding: 0; font-family: \'arial\', monospace; font-size: 13px; font-weight: normal; color: #000; list-style: none;">'.$this->data['fields']['branch']['street'].'</li>
            <li style="margin:0; padding: 0; font-family: \'arial\', monospace; font-size: 13px; font-weight: normal; color: #000; list-style: none;">'.$this->data['fields']['branch']['zip'].'</li>
            <li style="margin:0; padding: 0; font-family: \'arial\', monospace; font-size: 13px; font-weight: normal; color: #000; list-style: none;">Telefon:+47 '.$this->data['fields']['branch']['phone'].'</li>
            <li style="margin:0; padding: 0; font-family: \'arial\', monospace; font-size: 13px; font-weight: normal; color: #000; list-style: none;">Org.nr: '.$this->data['fields']['branch']['org_nr'].'</li>

          </ul></th>
      </tr>
      <tr>
        <td width="50%" valign="top" align="center" style="padding:0 0px 13px 0;"><ul style="margin:0; padding: 0; font-family: \'arial\', monospace; font-size: 13px; font-weight: normal">
      ...
      <tr>
			<td valign="top" align="left" style="padding:0 0 13px 0;"><ul style="margin:0; padding:0 0 0 2px; font-family: \'arial\', monospace; font-size: 13px; font-weight: normal">
      ...
      <tr>
        <td  valign="top" align="center"><table width="100%" align="center" border="0" cellpadding="0" cellspacing="0">
            <thead>
              <tr>
                <th style="text-align: left; width:18%; border-top:#000 dashed 1px; border-bottom:#000 dashed 1px; padding: 10px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Ant.</th>
                <th style="text-align: left; width:70%; border-top:#000 dashed 1px; border-bottom:#000 dashed 1px; padding: 10px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal ">Artikler</th>
                <th style="text-align: right; width:20%; border-top:#000 dashed 1px; border-bottom:#000 dashed 1px; padding: 10px 0; font-family: \'arial\', monospace; font-size: 13px; width:20%; font-weight:normal ">Beløp</th>
	...
	$commonfooter='</tbody>
			 <tr>
              <td colspan="3" nowrap="nowrap" style="text-align: center; padding: 5px 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;  vertical-align: top;">&nbsp;</td>
            </tr>
			</tbody>
          </table></td>
      </tr>
        <tr>
              <td colspan="3" valign="top" align="center" style="text-align: center; padding: 13px; font-family: \'arial\', monospace; font-size: 25px; font-weight:normal;">Intern Kopi</td>
            </tr>
    </tbody>
  </table>
';
		...
		$categoryprint='';
			...
			<tr>
				  <td colspan="3" style="margin:0px 0; padding:10px 0; font-family: \'arial\', monospace; font-size: 20px; font-weight: bold;  color: #000" valign="top" align="left">Ferdig: </td>
					...
					<td valign="top" colspan="3" align="left" style="margin:0px 0; padding:10px 0; font-family: \'arial\', monospace; font-size: 20px; font-weight: bold;  color: #000">'.$catdelivery.'</td>
					</tr>';
			...
			/* <tr>
              <td colspan="3" valign="top" align="center" style="text-align: center; padding: 13px; font-family: 'Droid Sans Mono', monospace; font-size: 12px; font-weight:normal;">[ Intern kopi ] <br> [ Kundens kvittering ]<br> [ Ekstra kopi (3) ]</td>
            </tr>*/
		...
		//	echo '<pre>';print_r($orderdetails);exit;
			...
			for($i=0;$i< count($orderdetails);$i++)
			{
			  ...
			  <tr><td colspan="3" style="text-align: left; padding: 5px 0 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;vertical-align: top; ">';
		...
		// echo '<pre>';print_r($barcodes);exit;
			 ...
			 $count=0;
			  foreach($barcodes as $baritems)
			  {
				...
				$str.='<tr>
                <td style="text-align: left; padding: 5px 0 0; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;vertical-align: top; ">'.$name.' <br>HS #'.$baritems['barcode'].' 
		...
		$frakt=$delviery;
			...
			{
				 $this->data['lists']['delivery_note'] = '<tr>
				  <td valign="top" colspan="3" align="left" style="margin:0; padding:0; font-family: \'arial\', monospace; font-size: 15px; font-weight: bold;  color: #000">Notater</td>
				</tr>
				<tr>
				  <td colspan="3" valign="top" align="left" style="text-align: left; padding: 10; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;">'.$this->data['fields']['order']['delivery_note'].'</td>
				</tr>';
			...
			{
				 $this->data['lists']['special_instruction'] = '<tr>
				  <td valign="top" colspan="3" align="left" style="margin:0; padding:0; font-family: \'arial\', monospace; font-size: 15px; font-weight: bold;  color: #000">Spesialinstruksjoner</td>
				</tr>
				<tr>
				  <td colspan="3" valign="top" align="left" style="text-align: left; padding: 10; font-family: \'arial\', monospace; font-size: 13px; font-weight:normal;">'.$this->data['fields']['order']['special_instruction'].'</td>
				</tr>';
                    ...
                    <div class="col-md-12 no-padd">
                      <div class="row mt-sm">
                        ...
                        <div class="pull-left col-md-4 no-padd mt-sm">
                          <button type="submit" id="add_productbtn"  style="font-size:14px !important;width:70%;padding:8px 0" class="btn-lg btn-primary">Add</button>
	...
	function _getcustomerinfo()
	{
	 ...
	 function __utleveringinfo()
	 {
		...
		for($i=0;$i<count($this->data['blocks']['ready']);$i++){
			 ...
			 if($result){		  
				...
				$str .='<a href="javascript:void(0);" onclick="ajaxutleveringedit('.$this->data['blocks']['ready'][$i]['id'].')" rel="'.$this->data['blocks']['ready'][$i]['id'].'">
				  <div class="col-md-2" id="orderinfo_'.$this->data['blocks']['iporders'][$i]['id'].'"> #'.$this->data['blocks']['ready'][$i]['id'].' </div>
			...
			if($result)
			{
				...
				$artikler = $this->general_model->trim_text($artikler);
				  ...
				  $status = ($opstatus=='canceled') ? 'C' : (($opstatus == 'paid') ?  'BE' : 'UB');	
					...
					$str .='<a  href="javascript:void(0);" onclick="ajaxutleveringedit('.$this->data['blocks']['iporders'][$i]['id'].')" rel="'.$this->data['blocks']['iporders'][$i]['id'].'">
					  <div class="col-md-3" id="orderinfo_'.$this->data['blocks']['iporders'][$i]['id'].'"> #'.$this->data['blocks']['iporders'][$i]['id'].' <span class="green-text">'.$branch.'</span></div>
					  <div class="col-md-2 no-padd"> '.$this->data['blocks']['iporders'][$i]['odate'].' </div>
					  <div class="col-md-3"> '.$kategori.' </div>';				  
					 ...
					 /*$str .='<div class="col-md-1 text-center"> <div class="'.$color.' paymentstatus"> '.$status.'</div> </div></a>*/
					 $str .='<div class="col-md-1 text-center"><div title="utsikt" class="bg-purple paymentstatus utsikt" id="'.$this->data['blocks']['iporders'][$i]['id'].'">UT</div></div>';
					$str .='	<div style="display:none;" id="kasserer_'.$this->data['blocks']['iporders'][$i]['id'].'">Kasserer: '.$this->data['fields']['employee']['initial'].'</div></div>';				
			}	
		  }//for
		   ...
		   $branch =  ($company == $this->data['settings_company']['company_name']) ?  '('.substr($company,0,3) .')':  (($company != $this->session->userdata['partner_branch_name']) ? '('.substr($company,0,3).')' :  ''); 
			...
			$artikler = $this->general_model->trim_text($artikler);
			  ...
			  $status = ($opstatus=='canceled') ? 'C' : (($opstatus == 'paid') ?  'BE' : 'UB');	
				...
				$str .='<a  href="javascript:void(0);" onclick="ajaxutleveringedit('.$this->data['blocks']['delivered'][$i]['id'].')" rel="'.$this->data['blocks']['delivered'][$i]['id'].'">
		...
		$html='<h2 class="title">Aktuelle</h2>
           <div class="finished_orders order-black aktuelle">
				...
				$html .='<div class="col-md-3 black-text bold">Ordre nr. </div>
		...
		{	
			foreach($_POST['orders'] as $orderid)
			{
				...
				$htmlout.='<div class="orderlisting row">
				 ...
				 {
					$htmlout.='<div class="col-md-4 black-text text-center no-padd bold">kr 0,00</div>';
				 }
					 ...
					 $prodtype=$this->process_order_model->validateProducttype($olineitmes['product']);
					  ...
					  else
					  {
						...
						if($qty > 0)
						{
							...
							{
								$htmlout.='<table width="276"  align="center" border="0" cellpadding="0" cellspacing="0">
							<tbody>
							  <tr>
								<td width="50%" valign="top" align="left" style="padding:0 0 0px 0;">
								<ul style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal">
									<li style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 22px; font-weight: normal; color: #000; list-style: none;">'.$orderid.'</li></ul>  
								</td>    
								<td width="50%" valign="top" align="left" style="padding:0 0 4px 0;"><ul style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal">
				
									<li style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal; color: #000; list-style: none; line-height:14px">'.$orderid.' '.$companyname.' '.$partnerinfo['id'].'</li>
									<li style="margin:0; padding: 0 0 0px 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal; color: #000; list-style: none; line-height:14px">'.$customer['customer_name'].'</li>
								</ul>     
							   </td>
							   </tr>
								<tr>
								<td width="50%" valign="top" align="left" style="padding:0 0 0px 0;">
								<ul style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal">
									<li style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 22px; font-weight: normal; color: #000; list-style: none;">'.$partnerinfo['id'].':'.$ready.'</li></ul>
								</td>    
								<td width="50%" valign="top" align="left" style="padding:0 0 0px 0;" ><ul style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal">
									<li style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal; color: #000; list-style: none;line-height:14px">I '.date('d/m',strtotime($orderinfo['order_time'])).'</li>
									<li style="margin:0; padding: 0 0 5px 0;  font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal; color: #000; list-style: none;line-height:14px">Rdy '.$ready.'</li>
								</ul>     
							   </td>
							   </tr>
							<tr>
								<td width="50%" valign="top" align="left" style="padding:0 0 0px 0;">
								<ul style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal">
									<li style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 22px; font-weight: normal; color: #000; list-style: none;">'.$weekday.':'.$item.'/'.$total_qty.'</li></ul>
								</td>    
								<td width="50%" valign="top" align="left" style="padding:0 0 0px 0;" ><ul style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal">
									<li style="margin:0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal; color: #000; list-style: none;line-height:14px">'.$olineitmes['name'].'</li>
									<li style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal; color: #000; list-style: none;line-height:14px">'.$customer['number'].'</li>
								</ul>     
							   </td>
							   </tr>
				 ...
				 $prodtype=$this->process_order_model->validateProducttype($olineitmes['product']);
				  ...
				  else
				  {
					...
					if($qty > 0)
					{
						...
						{
							$htmlout.='
					  <table width="276"  align="center" border="0" cellpadding="0" cellspacing="0">
						<tbody>
						  <tr>
							<td width="50%" valign="top" align="left" style="padding:0 0 0px 0;">
							<ul style="margin:0; padding: 0; font-family: \'Droid Sans Mono\', monospace; font-size: 12px; font-weight: normal">
	 ...
	 function __cancelorderlines()
	 {
		...
		{	
			...
			$result = array("status"=>'success','orderid'=>$orderid,"message"=>"Orderlines has been successfully canceled .");
