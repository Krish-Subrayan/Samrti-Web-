...
class Quotation extends MY_Controller
{
    ...
    function __saveQuotation()
    {
        ...
        if ($next) {
            ...
            $vars['company_name'] = $this->input->post('quotations_company_name');
            $vars['contact_name'] = $this->input->post('quotations_name');
            $vars['contact_email'] = $this->input->post('quotations_email');
            $vars['contact_telephone'] = $this->input->post('quotations_telephone');
            $vars['quotation_form_name'] = $this->input->post('quotations_form_title');
            $vars['quotation_id'] = $result;

            //email - team
            $this->__emailer('new_quotation_admin', $vars);
            /** --------------------------emailer - v2----------------------------------------------------------*/
    ...
    function __emailer($email = '', $vars = array())
    {
        ...
        //new quaotion - admin-------------------------------
        if ($email == 'new_quotation_admin') {

            //get message template from database
            $template = $this->settings_emailtemplates_model->getEmailTemplate('new_quotation_admin');
            ...
            for ($i = 0; $i < count($this->data['vars']['mailinglist_admins_full']); $i++) {
                ...
                $vars['url_quotation'] = $this->data['vars']['site_url_admin'] . '/quotation/view/'.$vars['quotation_id'];
